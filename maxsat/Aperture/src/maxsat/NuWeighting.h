...
class NUWEIGHTING {
 public:
  ...
  void soft_smooth_weights(long long &time_steps);
  void hard_increase_weights(long long &time_steps);
  void soft_increase_weights(long long &time_steps);
  void update_clause_weights(long long &time_steps);
  void unsat(int clause);
  void sat(int clause);
  void init(vector<int> &init_solution);
  void flip(int flipvar, long long &time_steps);
  void flip2(int flipvar, long long &time_steps);
  void update_goodvarstack1(int flipvar, long long &time_steps);
  void update_goodvarstack2(int flipvar);
  int pick_var(long long &time_steps);
  void update_clause_scores(long long s, long long &time_steps);
  long long floorToPowerOfTen(double x);
  long long closestPowerOfTen(double num);

  void soft_increase_weights_partial();
  void soft_increase_weights_not_partial(long long &time_steps);
...
inline void NUWEIGHTING::settings() {
  ...
  lambda = 0.15;
  backward_step = 54;
  max_clause_score = 81 * 20;
  gamma = 0.72;
  cut_round = 28000000000;
  improve_ratio = 0.067;
  if (1 == problem_weighted) {
    ...
    if (num_hclauses > 0)  // weighted partial
    {
      ...
      // hd_count_threshold = p1_hd_count_threshold; // 50;
      hd_count_threshold = 11;
      // rdprob = p1_rdprob; // 0.036;
      rdprob = 0.148;
      // rwprob = p1_rwprob; // 0.48;
      rwprob = 0.492;
      soft_smooth_probability = 2E-6;
      smooth_probability = 2E-5;
      softclause_weight_threshold = 50;

      // soft_increase_ratio = p1_soft_increase_ratio; // 1.001;
      soft_increase_ratio = 1.00144;
      if (num_sclauses == 0) {
        coe_tuned_weight = 1.0 / (1 + double(top_clause_weight - 1) /
                                          (double)(num_sclauses + 1));
      } else
        coe_tuned_weight =
    ...
    {
      hd_count_threshold = 48;
      // hd_count_threshold = p2_hd_count_threshold; // 50;
      coe_soft_clause_weight = 1;
      rdprob = 0.089;
      // rdprob = p2_rdprob; // 0.079;
      rwprob = 0.091;
      // rwprob = p2_rwprob; // 0.087;
      soft_smooth_probability = 1E-5;
      softclause_weight_threshold = 500;
      smooth_probability = 1E-4;
      soft_increase_ratio = 1.00003;
      // soft_increase_ratio = p2_soft_increase_ratio; // 1.00072;
...
inline void NUWEIGHTING::allocate_memory() {
  ...
  selected_clauses = new int[100];
  selected_times = new int[malloc_clause_length];
  sampled_clauses = new int[50];
...
inline void NUWEIGHTING::build_instance(
    int numVars, int numClauses, unsigned long long topClauseweight,
    clauselit **nuweighting_clause, int *nuweighting_clause_lit_count,
    unsigned long long *nuweighting_clause_weight) {
  istringstream iss;
  string line;
  char tempstr1[10];
  ...
  logger_.Log(VerbosityLevel::VVERBOSE, "build instime is {}", get_runtime());
...
inline void NUWEIGHTING::hard_increase_weights(long long &time_steps) {
  int i, c, v;
...
inline void NUWEIGHTING::soft_increase_weights(long long &time_steps) {
  int i, c, v;
...
inline void NUWEIGHTING::soft_smooth_weights(long long &time_steps) {
  int i, clause, v;
...
inline void NUWEIGHTING::update_clause_weights(long long &time_steps) {
  if (num_hclauses > 0) {
    // update hard clause weight
    hard_increase_weights(time_steps);
    if (soft_unsat_weight >= opt_unsat_weight) {
      soft_increase_weights(time_steps);
    }
  } else {
    if (((rand() % MY_RAND_MAX_INT) * BASIC_SCALE) < soft_smooth_probability &&
      ...
      soft_smooth_weights(time_steps);
    } else {
      soft_increase_weights_not_partial(time_steps);
    }
  }
}

inline void NUWEIGHTING::update_clause_scores(long long s,
                                              long long &time_steps) {
  int i, j;
  double stemp;

  long long opt = opt_unsat_weight;
  if (soft_unsat_weight < opt) opt = soft_unsat_weight;
  ...
  if (local_times < backward_step) {
    ...
    for (i = 0; i < backward_step; i++) {
      time_steps++;
      if (i == local_times % backward_step) continue;
...
inline int NUWEIGHTING::pick_var(long long &time_steps) {
  if (hardunsat_stack_fill_pointer + softunsat_stack_fill_pointer == 0) {
    return -1;
  }
  int i, v;
  int best_var;
  ...
  update_clause_weights(time_steps);
  ...
  if (hardunsat_stack_fill_pointer > 0) {
    ...
    if (num_clauses < ArmNum) {
      time_steps++;
      sel_c = softunsat_stack[rand() % softunsat_stack_fill_pointer];
    } else {
      while (1) {
        sampled_clauses[0] =
            softunsat_stack[rand() % softunsat_stack_fill_pointer];
        time_steps += 2;
        if (clause_lit_count[sampled_clauses[0]] != 0) break;
      }

      double min = clause_score[sampled_clauses[0]],
             max = clause_score[sampled_clauses[0]];
      time_steps++;
      ...
      for (int i = 1; i < ArmNum; i++) {
        while (1) {
          sampled_clauses[i] =
              softunsat_stack[rand() % softunsat_stack_fill_pointer];
          time_steps += 2;
          if (clause_lit_count[sampled_clauses[i]] != 0) break;
        }
        time_steps += 2;
        // cout << sampled_clauses[i] << endl;
      ...
      if (max == min) {
        ...
        for (int i = 1; i < ArmNum; i++) {
          ...
          } else if (selected_times[sampled_clauses[i]] ==
        ...
        time_steps += 3;
        double max_value =
            clause_score[sampled_clauses[0]] +
        ...
        for (int i = 1; i < ArmNum; i++) {
          ...
          double dtemp =
              clause_score[sampled_clauses[i]] +
              lambda * sqrt((log(local_times + 1)) /
      ...
      selected_times[sel_c] = selected_times[sel_c] + 1;
      if (local_times > 0) {
        long long s = pre_unsat_weight - soft_unsat_weight;
        update_clause_scores(s, time_steps);
      }
...
inline void NUWEIGHTING::update_goodvarstack2(int flipvar)
{
    ...
    for (int i = 0; i < score_change_stack_fill_pointer; i++)
    {
        ...
        // cout << "v is " << v << " score change time is " <<
score_change_time[v] << endl; if (!if_score_change[v]) continue;
...
*/
inline void NUWEIGHTING::flip(int flipvar, long long &time_steps) {
  int i, v, c, clen = 0;
  int index;
  clauselit *clause_c;
  score_change_stack_fill_pointer = 0;
...
inline void NUWEIGHTING::flip2(int flipvar, long long &time_steps) {
  int i, v, c;
  int index;
  clauselit *clause_c;
  ...
  score[flipvar] = -org_flipvar_score;
  time_steps++;
  update_goodvarstack1(flipvar, time_steps);
}
...
inline bool NUWEIGHTING::verify_sol() {
  ...
  for (c = 0; c < num_clauses; ++c) {
    ...
    if (flag == 0) {
      if (org_clause_weight[c] == top_clause_weight)  // verify hard clauses
      {
        ...
        std::ostringstream clause_stream;
        for (j = 0; j < clause_lit_count[c]; ++j) {
          if (clause_lit[c][j].sense == 0) clause_stream << "-";
          clause_stream << clause_lit[c][j].var_num << " ";
        }
        logger_.Log(VerbosityLevel::VVERBOSE, "{}", clause_stream.str());

        clause_stream.str("");
        clause_stream.clear();
        for (j = 0; j < clause_lit_count[c]; ++j)
          clause_stream << cur_soln[clause_lit[c][j].var_num] << " ";
        logger_.Log(VerbosityLevel::VVERBOSE, "{}", clause_stream.str());
        return 0;
      } else {
  ...
  if (verify_unsat_weight == opt_unsat_weight) {
    logger_.Log(VerbosityLevel::VVERBOSE, "yes {}", verify_unsat_weight);
  } else {
    logger_.Log(VerbosityLevel::VVERBOSE,
...
inline bool NUWEIGHTING::verify_goodvarstack(int flipvar) {
  for (int i = 1; i <= num_vars; ++i) {
    ...
    if (score[i] > 0 && already_in_goodvar_stack[i] == -1) {
      logger_.Log(VerbosityLevel::VVERBOSE, "wrong 1 :");
      logger_.Log(VerbosityLevel::VVERBOSE, "var is {}", i);
    } else if (score[i] <= 0 && already_in_goodvar_stack[i] != -1) {
      logger_.Log(VerbosityLevel::VVERBOSE, "wrong 2 :");
      logger_.Log(VerbosityLevel::VVERBOSE, "var is {}", i);
    }
    /*if (if_score_change[i] != 0)
    {
        logger_.Log(VerbosityLevel::VVERBOSE, "wrong 3 :");
        logger_.Log(VerbosityLevel::VVERBOSE, "var is {}", i);
    }*/
  }
  if (score[flipvar] > 0 && already_in_goodvar_stack[flipvar] != -1) {
    logger_.Log(VerbosityLevel::VVERBOSE, "wrong flipvar in good var {}",
                flipvar);
    logger_.Log(VerbosityLevel::VVERBOSE, "{}", score[flipvar]);
  }
  return 1;
}
...
inline void NUWEIGHTING::simple_print() {
  if (best_soln_feasible == 1) {
    if (verify_sol() == 1)
      logger_.Log(VerbosityLevel::VVERBOSE, "{}\t{}", opt_unsat_weight,
                  opt_time);
    else
      logger_.Log(VerbosityLevel::VVERBOSE, "solution is wrong");
  } else
    logger_.Log(VerbosityLevel::VVERBOSE, "{}\t{}", -1, -1);
}
