...
class Wrapper : ApertureCaDiCaL::Learner {
  ApertureCaDiCaL::Solver *solver;
  std::vector<int> clause;

public:
  unsigned clauses;
  Wrapper (ApertureCaDiCaL::Solver *s) : solver (s), clauses (0) {
...
static void formula (ApertureCaDiCaL::Solver &solver) {
  for (int r = -1; r < 3; r += 2)
    for (int s = -1; s < 3; s += 2)
      for (int t = -1; t < 3; t += 2)
...
int main () {
  ApertureCaDiCaL::Solver ping, pong;
  ping.set ("log", 1), pong.set ("log", 1);
  ping.set ("congruence", 0), pong.set ("congruence", 0);
  ping.set ("sweep", 0), pong.set ("sweep", 0);
  ping.set ("otfs", 0), pong.set ("otfs", 0);
  ping.set ("factor", 0), pong.set ("factor", 0);
  ping.set ("fastelim", 0), pong.set ("fastelim", 0);
