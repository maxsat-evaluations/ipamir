...
int Internal::trivially_false_satisfiable () {
  ...
  // Skip if any variable has a forced positive phase
  for (auto idx : vars) {
    if (phases.forced[idx] > 0)
      return 0;
  }
...
int Internal::trivially_true_satisfiable () {
  ...
  // Skip if any variable has a forced negative phase
  for (auto idx : vars) {
    if (phases.forced[idx] < 0)
      return 0;
  }
...
int Internal::forward_false_satisfiable () {
  ...
  START:
    ...
    if (val (idx))
      continue;
    // Respect forced phase, default to false (-idx)
    int dec = -idx;
    if (phases.forced[idx] > 0)
      dec = idx;
    else if (phases.forced[idx] < 0)
      dec = -idx;
    if (lucky_propagate_discrepency (dec)) {
      if (unsat)
        return 20;
      else
        return unlucky (0);
...
int Internal::forward_true_satisfiable () {
  ...
  START:
    ...
    if (val (idx))
      continue;
    // Respect forced phase, default to true (idx)
    int dec = idx;
    if (phases.forced[idx] > 0)
      dec = idx;
    else if (phases.forced[idx] < 0)
