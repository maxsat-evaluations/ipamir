...
CraigTracer::CraigTracer ()
    : ApertureCaDiCaL::Tracer (), marked_history (), marked_lits (),
...
void CraigTracer::conclude_unsat (
    ApertureCaDiCaL::ConclusionType conclusion,
    const std::vector<int64_t> &proof_chain) {
  if (craig_interpolant) {
    delete craig_interpolant;
    craig_interpolant = 0;
  ...
  CraigData *interpolant = 0;
  if (conclusion == ApertureCaDiCaL::ConclusionType::CONFLICT) {
  ...
  } else if (conclusion == ApertureCaDiCaL::ConclusionType::ASSUMPTIONS) {
    ...
    interpolant = new CraigData (*craig_interpolants[proof_chain[0] - 1]);
  } else if (conclusion == ApertureCaDiCaL::ConclusionType::CONSTRAINT) {
