...
// Will replace by C++20's [[likely]]/[[unlikely]] attributes, once properly and
// consistently implemented in both GCC and VS
#ifdef _WIN32
#define likely(x) (x)
#define unlikely(x) (x)
#else
...
class CDynArray {
 public:
  CDynArray(size_t initCap = 0)
      : m_Cap(initCap),
        m_B(initCap > MaxCapacity || initCap == 0
                ? nullptr
                : (T*)malloc(initCap * TSize)) {}
  CDynArray(size_t initCap, unsigned char initVal)
      : m_Cap(initCap), m_B(InitAlloc(initCap, initVal)) {}
  // Copy constructor
...
#else
  // Preventing inlining is required to work around an apparent GCC bug,
  // manifesting itself in the following error: error: argument 2 range
  // [9223372036854775808, 18446744073709551608] exceeds maximum object size
  // 9223372036854775807 [-Werror=alloc-size-larger-than=] auto tmp =
  // (T*)realloc(m_B, s);
  ...
  void reserve_atleast(size_t newCap) { reserve_exactly(GetNewCap(newCap)); }
  ...
  void memset(unsigned char newVal, size_t startIndIncl, size_t endIndExcl) {
    ...
    std::memset((void*)(m_B + startIndIncl), newVal,
                (endIndExcl - startIndIncl) * TSize);
  }
  ...
  void memcpy(size_t outStartInd, size_t inpStartInd, size_t entriesToCopy) {
    ...
    assert(inpStartInd + entriesToCopy > inpStartInd &&
           inpStartInd + entriesToCopy <= m_Cap);
    assert(outStartInd + entriesToCopy > outStartInd &&
           outStartInd + entriesToCopy <= m_Cap);
    // No overlap
    assert(outStartInd >= inpStartInd + entriesToCopy ||
           inpStartInd >= outStartInd + entriesToCopy);
    std::memcpy(m_B + outStartInd, m_B + inpStartInd,
                entriesToCopy * sizeof(*m_B));
  }

  // Potentially-overlapping memmove
  void memmove(size_t outStartInd, size_t inpStartInd, size_t entriesToCopy) {
    if (outStartInd >= inpStartInd + entriesToCopy ||
        inpStartInd >= outStartInd + entriesToCopy) {
      ...
      assert(outStartInd + entriesToCopy > outStartInd &&
             outStartInd + entriesToCopy <= m_Cap);
      std::memmove(m_B + outStartInd, m_B + inpStartInd,
                   entriesToCopy * sizeof(*m_B));
    }
  }
  ...
  inline T* get_ptr_no_assert(size_t i = 0) { return m_B + i; }
  ...
  inline const size_t& cap() const { return m_Cap; }

  inline size_t memMb() const { return (m_Cap * TSize) / 1000; }

  inline const bool empty() const { return cap() == 0; }
  ...
  void RemoveGarbage(TUInd startInd, TUInd& endInd,
                     function<bool(TUInd clsInd)> IsChunkDeleted,
                     function<TUInd(TUInd clsInd)> ChunkEnd,
                     function<void(TUInd oldWlInd, TUInd newWlInd)>
                         NotifyAboutRemainingChunkMove = nullptr) {
    auto FindNextDeleted = [&](TUInd firstInd, TUInd lastInd,
                               function<bool(TUInd clsInd)> IsChunkDeleted,
                               function<TUInd(TUInd clsInd)> ChunkEnd,
                               TUInd toInd = numeric_limits<T>::max(),
                               function<void(TUInd oldWlInd, TUInd newWlInd)>
                                   NotifyAboutRemainingChunkMove = nullptr) {
      ...
      while (firstInd < lastInd && !IsChunkDeleted(firstInd)) {
        ...
        if (NotifyAboutRemainingChunkMove != nullptr) {
          NotifyAboutRemainingChunkMove(firstInd,
                                        toInd + firstInd - initFirstInd);
        }
    ...
    auto FindNextNotDeleted = [&](TUInd firstInd, TUInd lastInd,
                                  function<bool(TUInd clsInd)> IsChunkDeleted,
                                  function<TUInd(TUInd clsInd)> ChunkEnd) {
      while (firstInd < lastInd && IsChunkDeleted(firstInd)) {
    ...
    TUInd toInd(FindNextDeleted(startInd, endInd, IsChunkDeleted, ChunkEnd,
                                startInd, NotifyAboutRemainingChunkMove));
    // Find the first clause after the first deleted chunk and put its index in
    // fromInd
    ...
    while (fromInd < endInd) {
      TUInd fromIndEnd =
          FindNextDeleted(fromInd, endInd, IsChunkDeleted, ChunkEnd, toInd,
      ...
      toInd += copiedInds;
      fromInd =
          FindNextNotDeleted(fromIndEnd, endInd, IsChunkDeleted, ChunkEnd);
    }
    endInd = toInd;
  }

  static constexpr double MultiplierDef = 1.625;
  inline void SetMultiplier(double multiplier = MultiplierDef) {
    assert(multiplier >= 1.);
 ...
 protected:
  ...
  static constexpr size_t MaxCapacity =
      (std::numeric_limits<size_t>::max)() / TSize;
  T* InitAlloc(size_t initCap, unsigned char initVal) {
    // #topor: are there more page-fault-friendly ways to allocate: (1)
    // 0-initialized memory; (2) non-initialized memory? For non-initialized
    // memory, see:
    // https://stackoverflow.com/questions/56411164/can-i-ask-the-kernel-to-populate-fault-in-a-range-of-anonymous-pages
    // For 0-initialized initialized memory, first note that gcc now rewrites
    // malloc & memset(0) into calloc. However, once this change in gcc has been
    // made, there was a substantial deterioration in the performance of a
    // random-access hash-table in FB (see
    // https://github.com/facebook/folly/issues/1508) One way to block this
    // optimization in gcc is as follows: auto p = malloc(n); asm volatile
    // ("":"+r"(p));
    // // another way to block GCC's optimization is: asm volatile
    // ("":::"memory"); memset(p, '\0', n); We're using calloc directly for now;
    // may want to explore other options later
  ...
  inline size_t GetNewCap(size_t cap) const {
    ...
    return newCapDouble > (double)(std::numeric_limits<size_t>::max)()
