...
bool CTopi<TLit, TUInd, Compress>::WLBinaryWatchExists(TULit l,
                                                       TULit otherWatch) {
  ...
  return find(binWatches.begin(), binWatches.end(), otherWatch) !=
         binWatches.end();
}

template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::WLRemoveBinaryWatch(TULit l,
                                                       TULit otherWatch) {
...
void CTopi<TLit, TUInd, Compress>::WLAddBinaryWatch(TULit l, TULit otherWatch) {
  ...
  // || watchArena == nullptr was added because of a Klocwork warning.
  // WLPrepareArena guarantees that IsUnrecoverable() holds if watchArena ==
  // nullptr.
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::WLSetCached(TULit l, TUInd clsInd,
                                               TULit cachedLit) {
  // Go over the long watches
  TWatchInfo& wi = m_Watches[l];
  for (auto [currLongWatchInd, currLongWatchPtr] =
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::WLReplaceInd(TULit l, TUInd clsInd,
                                                TUInd newClsInd) {
  // Go over the long watches
  TWatchInfo& wi = m_Watches[l];
  for (auto [currLongWatchInd, currLongWatchPtr] =
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::WLRemoveLongWatch(TULit l,
                                                     size_t longWatchInd) {
  assert(l < m_Watches.cap());
  ...
  if (longWatchInd != wi.m_LongWatches) {
    memcpy(&watchArena[wi.GetLongEntry(longWatchInd)],
           &watchArena[wi.GetLongEntries()], TWatchInfo::BinsInLongBytes);
  }
  ...
  switch (wi.m_BinaryWatches) {
    case 0:
      break;
    case 1:
      watchArena[wi.GetLongEntries()] =
          watchArena[wi.GetLongEntries() + TWatchInfo::BinsInLong];
      break;
    default:
      if constexpr (TWatchInfo::BinsInLong <= 2) {
        // Copy the binary watches from the end of the binary arena to the end
        // of the updated long arena
        memcpy(&watchArena[wi.GetLongEntries()],
               &watchArena[wi.GetUsedEntries()], TWatchInfo::BinsInLongBytes);
      } else {
        if (wi.m_BinaryWatches >= TWatchInfo::BinsInLong) {
          ...
          memcpy(&watchArena[wi.GetLongEntries()],
                 &watchArena[wi.GetUsedEntries() + TWatchInfo::BinsInLong -
                             wi.m_BinaryWatches],
                 wi.m_BinaryWatches * sizeof(TULit));
        }
      }
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::WLAddLongWatch(TULit l, TULit inlinedLit,
                                                  TUInd clsInd) {
  assert(l < m_Watches.cap());
  ...
  switch (wi.m_BinaryWatches) {
    case 0:
      break;
    case 1:
      watchArena[wi.GetUsedEntries() + LitsInInd] =
          watchArena[wi.GetLongEntries()];
      break;
    default:
      if constexpr (TWatchInfo::BinsInLong <= 2) {
        memcpy(&watchArena[wi.GetUsedEntries()],
               &watchArena[wi.GetLongEntries()], TWatchInfo::BinsInLongBytes);
      } else {
        if (wi.m_BinaryWatches >= TWatchInfo::BinsInLong) {
          memcpy(&watchArena[wi.GetUsedEntries()],
          ...
          memcpy(&watchArena[wi.GetUsedEntries() + TWatchInfo::BinsInLong -
                             wi.m_BinaryWatches],
                 &watchArena[wi.GetLongEntries()],
                 wi.m_BinaryWatches * sizeof(TULit));
        }
...
template <typename TLit, typename TUInd, bool Compress>
CTopi<TLit, TUInd, Compress>::TULit*
CTopi<TLit, TUInd, Compress>::WLPrepareArena(TULit l, bool allowNewBinaryWatch,
  ...
  auto AllocateNewArenaAndCopyOldArenaIfAny = [&](TUInd sz) -> TULit* {
    ...
    if (unlikely(allocatedWordsRequired < m_WNext) ||
        ((double)(m_WWasted + m_WNext) >
         (double)m_WNext * m_ParamMultWasteWatches)) {
      CompressWLs();
      allocatedWordsRequired = m_WNext + sz;
      if (unlikely(allocatedWordsRequired < m_WNext)) {
        SetStatus(TToporStatus::STATUS_INDEX_TOO_NARROW,
                  "WLPrepareArena: reached the end of the buffer");
        return nullptr;
      }
    }
    ...
    if (unlikely(m_W.uninitialized_or_erroneous())) {
      SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                "BReallocBeyond: couldn't reserve m_B");
      return nullptr;
    }
  ...
  const TUInd currRequiredEntries =
      (TUInd)allowNewBinaryWatch +
      ((TUInd)allowNewLongWatch * TWatchInfo::BinsInLong);
  if (unlikely(wi.IsEmpty())) {
    return AllocateNewArenaAndCopyOldArenaIfAny(
        m_ParamInitEntriesPerWL < currRequiredEntries
            ? bit_ceil(currRequiredEntries)
            : m_ParamInitEntriesPerWL);
  }
  ...
  if (unlikely(requiredEntries < wi.GetUsedEntries())) {
    // Overflow
    SetStatus(TToporStatus::STATUS_INDEX_TOO_NARROW,
              "The actual number of entries for one literal doesn't fit into "
              "the buffer");
    return nullptr;
  }
  ...
  if (unlikely(wi.m_AllocatedEntries == TWatchInfo::MaxWatchInfoAlloc)) {
    ...
    SetStatus(TToporStatus::STATUS_INDEX_TOO_NARROW,
              "The watch list for one literal doesn't fit into the buffer");
    return nullptr;
  }
...
/*bool CTopi<TLit,TUInd,Compress>::WLAssertConsistency(bool
testMissedImplications)
{
...

        {
                ...
                if (binaryWatches != 0)
                {
                        TSpanTULit binWatches = m_W.get_span_cap(wi.m_BInd +
wi.GetLongEntries(), wi.m_BinaryWatches); for (TULit secondLit : binWatches)
                        {
                                assert(secondLit != 0);
                                assert(secondLit < GetNextLit());
                                TBinCls clsBinCls = make_pair(l, secondLit);
                                array<TULit, 2> clsArray = { clsBinCls.first,
clsBinCls.second }; TSpanTULit cls(clsArray); if (testMissedImplications &&
(IsFalsified(cls[0]) || IsFalsified(cls[1])))
                                {
                                        if (IsFalsified(cls[0]) &&
IsFalsified(cls[1])) cout << "***ASSERTION-FAILURE FF-for-binary at " <<
SLits(cls) << endl << STrail() << endl; assert(!(IsFalsified(cls[0]) &&
IsFalsified(cls[1])));

                                        if (!(IsSatisfied(cls[0]) ||
IsSatisfied(cls[1]))) cout << "***ASSERTION-FAILURE
one-falsified-no-satisfied-for-binary at " << SLits(cls) << endl << STrail() <<
endl; assert(IsSatisfied(cls[0]) || IsSatisfied(cls[1]));

                                        const TULit satLit =
cls[IsSatisfied(cls[1])]; const TULit falseLit = cls[IsFalsified(cls[1])];
                                        assert(satLit != falseLit);

                                        if (!(GetAssignedDecLevel(satLit) <=
GetAssignedDecLevel(falseLit))) cout << "***ASSERTION-FAILURE
one-falsified-satisfied-higherdl-for-binary at " << SLits(cls) << endl <<
STrail() << endl; assert(GetAssignedDecLevel(satLit) <=
GetAssignedDecLevel(falseLit));
                                }
                        }
                }
        }
  ...
  for (TUVar v = m_TrailStart; v != BadUVar; v = m_VarInfo[v].m_TrailNext) {
    ...
    if (binaryWatches != 0) {
      TSpanTULit binWatches =
          b.get_span_cap(wi.m_WBInd + wi.GetLongEntries(), wi.m_BinaryWatches);
      for (TULit secondLit : binWatches) {
        ...
        if (IsFalsified(cls[0]) || IsFalsified(cls[1])) {
          if (IsFalsified(cls[0]) && IsFalsified(cls[1]))
            cout << "***ASSERTION-FAILURE FF-for-binary at " << SLits(cls)
                 << endl
                 << STrail() << endl;
          assert(!(IsFalsified(cls[0]) && IsFalsified(cls[1])));

          if (!(IsSatisfied(cls[0]) || IsSatisfied(cls[1])))
            cout << "***ASSERTION-FAILURE "
                    "one-falsified-no-satisfied-for-binary at "
                 << SLits(cls) << endl
                 << STrail() << endl;
          ...
          if (!(GetAssignedDecLevel(satLit) <= GetAssignedDecLevel(falseLit)))
            cout << "***ASSERTION-FAILURE "
                    "one-falsified-satisfied-higherdl-for-binary at "
                 << SLits(cls) << endl
                 << STrail() << endl;
    ...
    for (auto [currLongWatchInd, currLongWatchPtr] =
      ...
      if constexpr (!Compress) {
        if (!(clsInd >= m_FirstLearntClsInd || !ClsGetIsLearnt(clsInd)))
          cout << "***ASSERTION-FAILURE First-Learnt at " << SLits(cls) << endl
               << STrail() << endl;
      ...
      if (find(cls.begin(), cls.end(), cachedLit) == cls.end()) {
        cout << "***ASSERTION-FAILURE CL- at cached " << SLit(cachedLit)
             << "; cls = " << SLits(cls) << endl
             << STrail() << endl;
        assert(0);
      }
      ...
      if (IsFalsified(cls[0]) || IsFalsified(cls[1])) {
        ...
        const TUV otherLitDecLevel =
            IsAssigned(otherLit) ? GetAssignedDecLevel(otherLit) : BadUVar;
        ...
        if (IsSatisfied(otherLit)) {
          ...
          if (!(otherLitDecLevel <= falseLitDecLevel || it != cls.end()))
            cout << "***ASSERTION-FAILURE FS at " << SLits(cls) << endl
                 << STrail() << endl;
        ...
        if (!IsAssigned(otherLit)) {
          ...
          if (!(it != cls.end()))
            cout << "***ASSERTION-FAILURE F- at " << SLits(cls) << endl
                 << STrail() << endl;
          assert(it != cls.end());
        }
        ...
        if (IsFalsified(otherLit)) {
          auto it = find_if(cls.begin() + 2, cls.end(), [&](TULit l) {
            return IsSatisfied(l) &&
                   GetAssignedDecLevel(l) <= falseLitDecLevel &&
                   GetAssignedDecLevel(l) <= otherLitDecLevel;
          });
          if (!(it != cls.end()))
            cout << "***ASSERTION-FAILURE FF at " << SLits(cls) << endl
                 << STrail() << endl;
          assert(it != cls.end());
        }
      }
...
template <typename TLit, typename TUInd, bool Compress>
bool CTopi<TLit, TUInd, Compress>::WLAssertConsistency(
    bool testMissedImplications) {
  ...
  assert(all_of(longCls2Watches.begin(), longCls2Watches.end(),
                [&](const pair<TUInd, uint8_t>& p) { return p.second == 2; }));
  ...
  for (const TBinCls& binCls : binClssTwice) {
    ...
    assert(find(binClssTwice.begin(), binClssTwice.end(), binClsReversed) !=
           binClssTwice.end());
  }
...
template <typename TLit, typename TUInd, bool Compress>
bool CTopi<TLit, TUInd, Compress>::WLIsLitBetter(TULit lCand,
                                                 TULit lOther) const {
  const auto lCandWLEntries =
      m_ParamBCPWLChoice == 0   ? m_Watches[lCand].GetUsedEntries()
      : m_ParamBCPWLChoice == 1 ? m_Watches[lOther].GetUsedEntries()
                                : 1;
  const auto lOtherWLEntries =
      m_ParamBCPWLChoice == 0   ? m_Watches[lOther].GetUsedEntries()
      : m_ParamBCPWLChoice == 1 ? m_Watches[lCand].GetUsedEntries()
                                : 0;
  ...
  if (IsSatisfied(lCand)) {
    ...
    return GetAssignedDecLevel(lCand) < GetAssignedDecLevel(lOther) ||
           (GetAssignedDecLevel(lCand) == GetAssignedDecLevel(lOther) &&
            lCandWLEntries < lOtherWLEntries);
