...
class CVarScores {
 public:
  // The heap's 0 index must always be occupied, so that position 0 would mean
  // not-in-the-heap
  CVarScores(double& varActivityInc)
      : m_VarActivityInc(varActivityInc), m_Heap(1, 0, 1) {
    static_assert(std::is_same<TUVar, TUV>::value);
  }

  void SetInitOrder(bool initOrder) { m_InitOrder = initOrder; }
  ...
  inline bool uninitialized_or_erroneous() const {
    return m_PosScore.uninitialized_or_erroneous() ||
           m_Heap.uninitialized_or_erroneous();
  }
  ...
  inline bool in_heap(TUVar v) const {
    assert(v < m_PosScore.cap());
    return m_PosScore[v].m_Pos > 0;
  }
  ...
  inline TUVar get_min() const { return m_Heap[1]; }
  ...
  // Use set_var_score only if rebuild is surely scheduled soon, otherwise it
  // will botch the data structure!
  inline void set_var_score(TUVar v, double newScore) {
    m_PosScore[v].m_Score = newScore;
  }

  inline void var_inc_update(double varDecay) {
    m_VarActivityInc *= (1. / varDecay);
  }

  inline void set_multiplier(double multiplier) {
    m_PosScore.SetMultiplier(multiplier);
    m_Heap.SetMultiplier(multiplier);
  }

  inline void replace_pos_score_vars(TUVar vFrom, TUVar vTo) {
    m_PosScore[vTo] = move(m_PosScore[vFrom]);
  }

  inline size_t memMb() const {
    return m_Activity.memMb() + m_Heap.memMb() + m_PosScore.memMb();
  }
 ...
 protected:
  ...
  // false: bigger variable indices first; true: smaller variable indices first
  // (default) We do the simulation by providing two implementations of the
  // "better" predicate (see below) See the parameter m_ParamVsidsInitOrder for
  // more details
  ...
  // Cannot put the static asserts in the class itself, since it's forbidden by
  // C++ standard as the functions are undefined inside the class
  ...
  void percolate_down(TUV i) {
    ...
    while (left(i) < m_Heap.size()) {
      auto child =
          right(i) < m_Heap.size() && better(m_Heap[right(i)], m_Heap[left(i)])
              ? right(i)
              : left(i);
  ...
  inline bool better(TUVar v1, TUVar v2) const {
    return m_InitOrder ? m_PosScore[v1].m_Score > m_PosScore[v2].m_Score
                       : m_PosScore[v1].m_Score >= m_PosScore[v2].m_Score;
  }
};
}  // namespace ApertureTopor
