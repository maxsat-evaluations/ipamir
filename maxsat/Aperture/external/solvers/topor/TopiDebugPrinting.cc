...
string CTopi<TLit, TUInd, Compress>::SE2I() {
  ...
  for (TLit externalLit = 1; externalLit < (TLit)m_E2ILitMap.cap();
       ++externalLit) {
...
string CTopi<TLit, TUInd, Compress>::STrail() {
  ...
  for (TUVar v = m_TrailEnd; v != BadUVar; v = m_VarInfo[v].m_TrailPrev) {
    ...
    [[maybe_unused]] const TUV prevDecLevel =
        m_VarInfo[v].m_TrailPrev == BadUVar
            ? numeric_limits<TUV>::max()
            : GetAssignedDecLevelVar(m_VarInfo[v].m_TrailPrev);
...
void CTopi<TLit, TUInd, Compress>::PrintDebugModel(TToporReturnVal trv) {
  if (trv != TToporReturnVal::RET_SAT) {
    cout << "m_DebugModel = FAILED, since the return value is not SAT; it's "
         << trv << endl;
    return;
  }
  ...
  for (TLit v = 1; v <= m_Stat.m_MaxUserVar; ++v) {
    ...
    assert(externalVal == TToporLitVal::VAL_SATISFIED ||
           externalVal == TToporLitVal::VAL_UNSATISFIED);
    cout << ", "
         << (externalVal == TToporLitVal::VAL_SATISFIED ? "true" : "false");
    if (v % 100 == 0) {
      cout << "\n";
    }
  }
...
void CTopi<TLit, TUInd, Compress>::VerifyDebugModel() {
  ...
  auto SInternalLitData = [&](TULit l) {
    ...
    ss << "{index " << l << " : ilit " << SLit(l) << " : elit " << elit
       << "; debugModel = "
       << (elit > 0 ? m_DebugModel[elit] : m_DebugModel[-elit]) << "}";
  ...
  for (TLit externalV = 1; externalV <= m_Stat.m_MaxUserVar; ++externalV) {
    ...
    const TUVar v = GetVar(l);
    internalVarVals[v] = IsNeg(l) ? (uint8_t)!m_DebugModel[externalV]
                                  : (uint8_t)m_DebugModel[externalV];
    // cout << "m_DebugModel[" << externalV << "] = " << m_DebugModel[externalV]
    // << " : " << SLit(l) << endl;
  }
  ...
  for (TUVar v = m_TrailStart; v != BadUVar; v = m_VarInfo[v].m_TrailNext) {
    ...
    if (expectedVal != -1) {
      assert(IsSatisfied(l) == (bool)expectedVal ||
             P("\t" + SLit(GetLit(v, false)) + "\n"));
      if (IsSatisfied(l) != (bool)expectedVal) {
        cout << "VerifyDebugModel ERROR in verifying the trail at literal "
             << SInternalLitData(l) << ". Exiting...\n";
        exit(-1);
      }
    }
  }
  ...
  for (TULit l = 1; l < GetNextLit(); ++l) {
    ...
    if (wi.m_BinaryWatches != 0) {
      TSpanTULit binWatches = m_W.get_span_cap(wi.m_WBInd + wi.GetLongEntries(),
                                               wi.m_BinaryWatches);
      for (TULit secondLit : binWatches) {
        const auto expectedValSecondLit = ExpectedVal(secondLit);
        assert(expectedValSecondLit != 0 ||
               P("\t" + SLit(l) + " " + SLit(secondLit) + "\n"));
        if (expectedValSecondLit == 0) {
          cout << "[l secondLit] = "
               << "[" << SInternalLitData(l) << ", "
               << SInternalLitData(secondLit) << "]" << endl;
          cout << "VerifyDebugModel ERROR in verifying a binary clause! "
                  "Exiting...\n";
          exit(-1);
        }
      }
  ...
  for (TUInd clsInd = ClsLoopFirst(false); !ClsLoopCompleted();
    ...
    if (!isOK) {
      ...
      cout << "\nVerifyDebugModel ERROR in verifying a long clause! "
              "Exiting...\n";
      exit(-1);
    }
  }
