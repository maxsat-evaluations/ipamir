...
void CTopi<TLit, TUInd, Compress>::ReserveVarAndLitData(size_t maxAssumps) {
  ...
  // (2) the current decision level (which might still be greater than
  // GetNextVar() + maxAssumps because of the previous call with potentially
  // more assumptions)
  const auto perDecLevelAlloc =
      std::max((size_t)GetNextVar() + (size_t)maxAssumps, (size_t)m_DecLevel) +
      1;
  ReserveExactly(m_Watches, GetNextLit(), 0,
                 "m_Watches in ReserveVarAndLitData");
  ReserveExactly(m_AssignmentInfo, GetNextVar(), 0,
                 "m_AssignmentInfo in ReserveVarAndLitData");
  if (m_PolarityInfoActivated)
    ReserveExactly(m_PolarityInfo, GetNextVar(), 0,
                   "m_PolarityInfo in ReserveVarAndLitData");
  ReserveExactly(m_VarInfo, GetNextVar(), 0,
                 "m_VarInfo in ReserveVarAndLitData");
  ReserveExactly(m_ToPropagate, GetNextVar(),
                 "m_ToPropagate in ReserveVarAndLitData");
  ReserveExactly(m_TrailLastVarPerDecLevel, perDecLevelAlloc, BadUVar,
                 "m_TrailLastVarPerDecLevel in ReserveVarAndLitData");
  ReserveExactly(m_VsidsHeap, GetNextVar(),
                 "m_VsidsHeap in ReserveVarAndLitData");
  ReserveExactly(m_HandyLitsClearBefore[0], GetNextVar(),
                 "m_HandyLitsCleanBefore[0] in ReserveVarAndLitData");
  if (m_ParamFlippedRecordingMaxLbdToRecord != 0)
    ReserveExactly(m_HandyLitsClearBefore[1], GetNextVar(),
                   "m_HandyLitsCleanBefore[1] in ReserveVarAndLitData");
  ReserveExactly(m_VisitedVars, GetNextVar(),
                 "m_VisitedVars in ReserveVarAndLitData");
  ReserveExactly(m_DecLevelsLastAppearenceCounter, perDecLevelAlloc, 0,
                 "m_DecLevelsLastAppearenceCounter in ReserveVarAndLitData");
  if (UseI2ELitMap())
    ReserveExactly(m_I2ELitMap, GetNextVar(), 0,
                   "m_I2ELitMap in ReserveVarAndLitData");
  if (IsCbLearntOrDrat())
    ReserveExactly(m_UserCls, GetNextVar(),
                   "m_UserCls in ReserveVarAndLitData");
  if (m_ParamOnTheFlySubsumptionParentMinGlueToDisable > 0)
    ReserveExactly(m_CurrClsCounters, GetNextVar(), 0,
                   "m_CurrClsCounters in ReserveVarAndLitData");
  if (m_ParamRestartStrategyInit == RESTART_STRAT_NUMERIC ||
      m_ParamRestartStrategyS == RESTART_STRAT_NUMERIC ||
      m_ParamRestartStrategyN == RESTART_STRAT_NUMERIC)
    ReserveExactly(m_RstNumericLocalConfsSinceRestartAtDecLevelCreation,
                   perDecLevelAlloc, 0,
                   "m_RstArithLocalConfsSinceRestartAtDecLevelCreation in "
                   "ReserveVarAndLitData");
  if (m_ParamCustomBtStratInit > 0 || m_ParamCustomBtStratS > 0 ||
      m_ParamCustomBtStratN > 0)
    ReserveExactly(m_BestScorePerDecLevel, perDecLevelAlloc, 0,
                   "m_ParamCustomBtStrat in ReserveVarAndLitData");
  ReserveExactly(m_HandleNewUserCls, GetNextVar(),
                 "m_HandleNewUserCls in ReserveVarAndLitData");
}
...
void CTopi<TLit, TUInd, Compress>::MoveVarAndLitData(TUVar vFrom, TUVar vTo) {
  ...
  if (m_PolarityInfoActivated)
    m_PolarityInfo[vTo] = move(m_PolarityInfo[vFrom]);
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::RecordDeletedLitsFromCls(
    TUV litsNum, uint16_t bitsForLit) {
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::ClsDelNewLearntOrGlueUpdate(TUInd clsInd,
                                                               TUV prevGlue) {
  if (m_ParamClsDelStrategy > 0) {
    ...
    if (currActivity > 1e20) {
      // Rescale:
      for (TUInd clsIndLocal = ClsLoopFirst(true); !ClsLoopCompleted();
           clsIndLocal = ClsLoopNext()) {
...
void CTopi<TLit, TUInd, Compress>::DeleteClausesIfRequired() {
  if (m_ParamClsDelStrategy == 0 || IsUnrecoverable() ||
      m_Status == TToporStatus::STATUS_USER_INTERRUPT ||
      (ClsDeletionTrigger() < m_ClsDelInfo.m_TriggerNext) ||
      (m_ParamClsDelDeleteOnlyAssumpDecLevel &&
  ...
  CApplyFuncOnExitFromScope<> onExit([&]() {
    ...
    if (unlikely(m_FirstLearntClsInd !=
                     numeric_limits<decltype(m_FirstLearntClsInd)>::max() &&
  ...
  static constexpr bool sizetGreaterThenActs =
      sizeof(size_t) > sizeof(decltype(m_Stat.m_ActiveLongLearntClss));
  const size_t learntsSz =
      sizetGreaterThenActs || m_Stat.m_ActiveLongLearntClss <
                                  (decltype(m_Stat.m_ActiveLongLearntClss))
                                      numeric_limits<size_t>::max()
          ? (size_t)m_Stat.m_ActiveLongLearntClss
          : (decltype(m_Stat.m_ActiveLongLearntClss))
                numeric_limits<size_t>::max();
  CVector<TUInd> learnts(learntsSz);
  if (learnts.uninitialized_or_erroneous()) {
    SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
              "CTopi<TLit,TUInd,Compress>::DeleteClausesIfRequired: couldn't "
              "allocate learnts");
    return;
  }
  ...
  for (TUVar v = m_TrailStart; v != BadUVar; v = GetTrailNextVar(v)) {
    ...
    // The "(vi.m_DecLevel > 0 || clsInd + 2 < m_B.cap())" part (and also "if
    // (vi.m_DecLevel > 0)" below) is a work-around, so that parents of globally
    // satisfied variables wouldn't be actually visited (with ClsSetSkipdel),
    // yet they will be counted for clause deletion heuristic. Skipping this
    // condition causes a correctness bug, since global parent aren't
    // maintained, while dealing with the global parents more aggressively
    // (e.g., nullifying them in Assign) hurts the clause deletion heuristic
    if (!ai.IsAssignedBinary() &&
        (vi.m_DecLevel > 0 || clsInd + 2 < m_B.cap()) && clsInd != BadClsInd &&
  ...
  for (TUInd clsInd = ClsLoopFirst(true); !ClsLoopCompleted();
       clsInd = ClsLoopNext()) {
    if (ClsChunkDeleted(clsInd) || !ClsGetIsLearnt(clsInd) ||
        ClsGetGlue(clsInd) <= m_ClsDelInfo.m_GlueNeverDelete) {
      if (!ClsChunkDeleted(clsInd) && ClsGetIsLearnt(clsInd)) {
  ...
  if (m_ClsDelInfo.m_Clusters == 0) {
    sort(learntsSpan.begin(), learntsSpan.end(),
         [&](TUInd clsInd1, TUInd clsInd2) {
           return ClsGetActivity(clsInd1) < ClsGetActivity(clsInd2) ||
                  ((ClsGetActivity(clsInd1) == ClsGetActivity(clsInd2)) &&
                   (ClsGetGlue(clsInd1) > ClsGetGlue(clsInd2)));
         });
  } else {
    sort(learntsSpan.begin(), learntsSpan.end(),
         [&](TUInd clsInd1, TUInd clsInd2) {
           const array<TUV, 2> glues = {ClsGetGlue(clsInd1),
                                        ClsGetGlue(clsInd2)};
           const array<uint8_t, 2> clusters = {
               m_ClsDelInfo.GetCluster(glues[0]),
               m_ClsDelInfo.GetCluster(glues[1])};
           if (clusters[0] != clusters[1]) {
             return clusters[0] > clusters[1];
           }

           const array<float, 2> acts = {ClsGetActivity(clsInd1),
                                         ClsGetActivity(clsInd2)};
           return acts[0] < acts[1] ||
                  (acts[0] == acts[1] && glues[0] > glues[1]);
         });
  }

  size_t iLastExcl = (decltype(iLastExcl))((float)(ClsDeletionTrigger() -
                                                   undeletableButNotTouched) *
                                           m_ClsDelInfo.m_FracToDelete);
  if (iLastExcl > learnts.size()) {
    iLastExcl = learnts.size();
  }

  if (m_ParamClsDelStrategy == 1) {
    m_ClsDelInfo.m_TriggerNext =
        ClsDeletionTrigger() + m_ClsDelInfo.m_TriggerInc;
    const double nextTriggerIncD =
        (double)m_ClsDelInfo.m_TriggerInc * m_ClsDelInfo.m_TriggerMult;
    ...
    const float nextChange = static_cast<float>(
        (m_Stat.m_Conflicts / m_ClsDelInfo.m_CurrChange) + 1.0);
    ...
    const double triggerNext =
        (double)nextChange * (double)m_ClsDelInfo.m_CurrChange;
    if (unlikely(
            triggerNext >
            (double)
                numeric_limits<decltype(m_ClsDelInfo.m_TriggerNext)>::max())) {
      m_ClsDelInfo.m_TriggerNext =
          numeric_limits<decltype(m_ClsDelInfo.m_TriggerNext)>::max();
    } else {
...
void CTopi<TLit, TUInd, Compress>::SimplifyIfRequired() {
  if (m_DecLevel > m_DecLevelOfLastAssignedAssumption ||
      m_TrailLastVarPerDecLevel[0] == m_LastGloballySatisfiedLitAfterSimplify ||
      m_ImplicationsTillNextSimplify > 0 || IsUnrecoverable() ||
      m_Status == TToporStatus::STATUS_USER_INTERRUPT) {
    return;
  }
  ...
  // We sift variable indices, if there are at least two globally assigned
  // variables We need to always leave one globally assigned variable to be able
  // to map external globally assigned variables to it
  const bool siftVarIndices =
      m_TrailLastVarPerDecLevel[0] != BadUVar &&
      GetTrailPrevVar(m_TrailLastVarPerDecLevel[0]) != BadUVar;
  ...
  if (siftVarIndices) {
    ...
    for (TUVar currV = m_TrailLastVarPerDecLevel[0]; currV != BadUVar;
         currV = GetTrailPrevVar(currV)) {
      MarkVisitedVar(currV);
    }

    // Sorting the visited, so that m_VisitedVars.back() would have the smallest
    // globaly assigned variable index
    ...
    assert(m_VisitedVars.size() > 0 &&
           globallySatifiedVarLowestIndex < m_VisitedVars.back());
    const TULit globallySatifiedLitLowestIndex =
        GetAssignedLitForVar(globallySatifiedVarLowestIndex);

    // m_HandyLitsClearBefore[0] will hold the new variable indices for the
    // sifted and removed (highest) variable indices
    m_HandyLitsClearBefore[0].memset(BadULit);

    auto GetGlobalSatLit = [&](TUVar v) {
      const TULit l = GetLit(v, false);
      return IsGloballySatisfied(l) ? globallySatifiedLitLowestIndex
                                    : Negate(globallySatifiedLitLowestIndex);
    };
    m_HandyLitsClearBefore[0][globallySatifiedVarLowestIndex] =
        GetGlobalSatLit(globallySatifiedVarLowestIndex);

    for (newLastExistingVar = m_LastExistingVar;
         (!m_VisitedVars.empty() &&
          m_VisitedVars.back() < newLastExistingVar) ||
         (IsGloballyAssignedVar(newLastExistingVar) &&
          m_HandyLitsClearBefore[0][newLastExistingVar] !=
              GetGlobalSatLit(newLastExistingVar));
         --newLastExistingVar) {
      if (IsGloballyAssignedVar(newLastExistingVar)) {
        m_HandyLitsClearBefore[0][newLastExistingVar] =
            GetGlobalSatLit(newLastExistingVar);
        assert(NV(1) ||
               P("\tGlobal variable removed: " + SVar(newLastExistingVar) +
                 " (mapped to " +
                 SLit(m_HandyLitsClearBefore[0][newLastExistingVar]) + ")\n"));
      } else {
        const TUVar toVar = VisitedPopBack();
        m_HandyLitsClearBefore[0][toVar] = GetGlobalSatLit(toVar);
        assert(NV(1) ||
               P("\tGlobal variable removed: " + SVar(toVar) + " (mapped to " +
                 SLit(m_HandyLitsClearBefore[0][toVar]) + ")\n"));
        m_HandyLitsClearBefore[0][newLastExistingVar] = GetLit(toVar, false);
        assert(NV(1) ||
               P("\tVariable re-indexed to literal: " +
                 SVar(newLastExistingVar) + " to " +
                 SLit(m_HandyLitsClearBefore[0][newLastExistingVar]) + "\n"));
      }
    }
    ...
    if (m_CurrCustomBtStrat > 0 && m_BestScorePerDecLevel.cap() != 0) {
      if (m_ParamSimplifyGlobalLevelScoreStrat == 0) {
        m_BestScorePerDecLevel[0] =
            m_VsidsHeap.get_var_score(globallySatifiedVarLowestIndex);
      } else if (m_ParamSimplifyGlobalLevelScoreStrat == 1) {
        m_BestScorePerDecLevel[0] = CalcMinDecLevelScore(0);
        m_VsidsHeap.set_var_score(globallySatifiedVarLowestIndex,
                                  m_BestScorePerDecLevel[0]);
      } else {
  ...
  assert(NV(1) ||
         P("Previous --> new last variable: " + to_string(m_LastExistingVar) +
           " --> " + to_string(newLastExistingVar) + "\n"));

  auto RetSiftedLit = [&](TULit l) {
    const TUVar v = GetVar(l);
    if (v > newLastExistingVar) {
      return IsNeg(l) ? Negate(m_HandyLitsClearBefore[0][v])
                      : m_HandyLitsClearBefore[0][v];
    } else {
      return l;
    }
  };
  ...
  // Delete globally falsified literals (that is, mark them for deletion for the
  // garbage collector) Delete globally satisfied clauses (that is, mark them
  // for deletion for the garbage collector) Move watches to the earliest
  // satisfied literals for clauses satisfied by assumptions Sift
  // assumption-falsified literals towards the end of the clause in clauses,
  // which are not satisfied Sift variable indices, if required

  unordered_set<TUInd> bcClssSimplifiedAndMovedToNewBuffer;
  for (TUInd clsInd = ClsLoopFirst(false); !ClsLoopCompleted();
       clsInd = ClsLoopNext()) {
    if constexpr (Compress) {
      if (bcClssSimplifiedAndMovedToNewBuffer.find(clsInd) !=
          bcClssSimplifiedAndMovedToNewBuffer.end()) {
        assert(NV(2) ||
               P("New clause " + HexStr(clsInd) +
                 " skipped, since had just been moved to this buffer: " +
                 SLits(Cls(clsInd)) + "\n"));
        continue;
      }
    }
    ...
    assert(NV(2) || P("New clause " + HexStr(clsInd) + ": " +
                      SLits(Cls(clsInd)) + "\n"));
    ...
    for (size_t i = 0; i < cls.size() && !isGloballySatisfied; ++i) {
      ...
      if (IsAssigned(currLit)) {
        ...
        const TUV decLevel = m_DecLevelOfLastAssignedAssumption == 0
                                 ? 0
                                 : GetAssignedDecLevel(currLit);
        if (isSatisfied) {
      ...
      if (siftVarIndices && !anySiftedVars &&
          GetVar(currLit) > newLastExistingVar) {
        anySiftedVars = true;
      }
    }
    ...
    if (globallyFalsifiedLitsNum > 0) {
      assert(NV(2) || P("\t" + to_string(globallyFalsifiedLitsNum) +
                        " globally falsified literals -- removing them from "
                        "the clause...\n"));
      assert(globallyFalsifiedLitsNum < cls.size());
      // Having all literals but one globally falsified is impossible at this
      // point, since then the remaining literal must have been a globally
      // satisfied one (since BCP guarantees no delayed implications at the
      // end), which would have made the clause deleted and the loop continued
      ...
      if (globallyFalsifiedLitsNum + 2 == cls.size()) {
        ...
        // The clause will now become binary, so we add a new binary clause and
        // delete this one
        auto it1 = find_if(cls.begin(), cls.end(), [&](const TULit l) {
          return !IsGloballyFalsified(l);
        });
        assert(it1 != cls.end());
        auto it2 = find_if(it1 + 1, cls.end(), [&](const TULit l) {
          return !IsGloballyFalsified(l);
        });
        assert(it2 != cls.end());
        assert(find_if(it2 + 1, cls.end(), [&](const TULit l) {
                 return !IsGloballyFalsified(l);
               }) == cls.end());

        array<TULit, 2> newBinCls = {*it1, *it2};
        array<TULit, 2> newBinClsSifted = {RetSiftedLit(newBinCls[0]),
                                           RetSiftedLit(newBinCls[1])};
        ...
        assert(NV(2) ||
               P("\tThe clause has become binary: " + SLits(newBinCls) + "\n"));
        if (unlikely(IsUnrecoverable())) return;

        continue;
      }

      // Now we need to remove the globally falsified literals and let the
      // clause be
      ...
      // Set the cached literals for watches unset by the previous loop to the
      // 2nd watch (they might be incorrect, because some literals have been
      // removed)
      for (uint8_t currWatchI = 0; currWatchI <= 1; ++currWatchI) {
      ...
      // remove_if makes sure globally falsified literals are no longer part of
      // the beginning of the clause, which is exactly what we need!
      [[maybe_unused]] auto itEndRemaining =
          remove_if(cls.begin() + 2, cls.end(),
                    [&](TULit l) { return IsGloballyFalsified(l); });
      ...
      if constexpr (!Compress) {
        ...
        // Renewing the span
        cls.Update();
        assert(NV(2) || P("\tAfter removing the globally falsified literals: " +
                          SLits(cls) + "\n"));
        ...
        // Updating the number of literal in long clauses
        RecordDeletedLitsFromCls(globallyFalsifiedLitsNum,
                                 ((TBCInd)clsInd).GetHashId().m_BitsForLit);

        const auto oldClsInd = clsInd;
        auto [deletionHandled, spareUsed] =
            BCDeleteLitsByMovingToOtherBufferIfRequiredAssumingLastDeleted(
                clsInd, (TUV)cls.size(), globallyFalsifiedLitsNum, true);
        if (!deletionHandled) {
          for (auto it = cls.begin() + (cls.size() - globallyFalsifiedLitsNum);
               it != cls.end(); ++it) {
            *it = BadULit;
          }
          assert(NV(2) || P("\tCompressed: after removing the globally "
                            "falsified literals: " +
                            SLits(cls) + "\n"));
          ClsSetSize(clsInd, (TUV)cls.size() - globallyFalsifiedLitsNum);
          cls.Update();
          ...
          // assert(NV(2) || P("\tCompressed: deletion handled by moving the
          // updated clause to another buffer: " + SLits(Cls(clsInd)) + "\n"));
        }
      }
    }
    ...
    if (isAssumpSatisfied) {
      ...
      // Make cls[1] contain the second lowest satisfied literal, if any
      {
        ...
        if (IsSatisfied(*lowestSatisfiedIt) &&
            lowestSatisfiedIt != cls.begin() + 1) {
      ...
      assert(NV(2) ||
             P("\tSatisfied by assumption propagation; the new clause: " +
               SLits(cls) + "\n"));
    }

    // If there are some assumption-falsified literals and the clause is not
    // assumption-satisfied, we sift the assumption-falsified literals towards
    // the end. Note that if the clause is assumption-satisfied, the rest of the
    // clause won't be visited anyway (with the current assumptions), so we pass
    // in that case
    if (assumpFalsifiedLitsNum > 0 && !IsSatisfied(cls[0]) &&
        !IsSatisfied(cls[1])) {
      // remove_if wouldn't work, since it doesn't move the removed elements
      // towards the end of the span so we created move_it, which is identical
      // to remove_it, expect that the  removed elements are moved towards the
      // end of the span, but their order is not preserved. The order of the
      // remaining elements is still preserved
      [[maybe_unused]] auto itEndRemaining = move_if(
          cls.begin() + 2, cls.end(), [&](TULit l) { return IsFalsified(l); });
      assert(cls.end() - itEndRemaining == assumpFalsifiedLitsNum);
      assert(NV(2) || P("\tSome literals falsified by assumption propagation; "
                        "the new clause: " +
                        SLits(cls) + "\n"));
    }
    ...
    if (siftVarIndices && anySiftedVars) {
      ...
      transform(cls.begin(), cls.end(), cls.begin(), [&](TULit l) {
        ...
        return v > newLastExistingVar
                   ? (isNeg ? Negate(m_HandyLitsClearBefore[0][v])
                            : m_HandyLitsClearBefore[0][v])
                   : l;
      });

      assert(NV(2) ||
             P("\tSifted some variable indices to: " + SLits(cls) + "\n"));
    }
  }
  ...
  if constexpr (Compress) {
    if (m_BCSpare.size() != 0) {
      ...
      m_BC.insert(make_move_iterator(m_BCSpare.begin()),
                  make_move_iterator(m_BCSpare.end()));
      m_BCSpare.clear();
    }
  }

  // Go over the watches: delete satisfied clauses
  // Remove the globally satisfied variables, except for one and reuse their
  // numbers by other variables. One globally satisfied variable is still
  // required to map globally satisfied external variables into it.

  // We Mark all the relevant positive literals and Root all the relevant
  // negative literals We also remove all the watches of the globally satisfied
  // literals (that is, the remaining binary watches, since the long ones have
  // already been removed), and mark the chunks as deleted
  ...
  for (TUVar currV = m_TrailLastVarPerDecLevel[0]; currV != BadUVar;
    ...
    if (!wi.IsEmpty()) {
      ...
      const TSpanTULit binWatches = TSpanTULit(
          m_W.get_ptr(wi.m_WBInd) + wi.GetLongEntries(), wi.m_BinaryWatches);
      ...
      // Removing all the watches (including binary watches) of the globally
      // satisfied literal
      MarkWatchBufferChunkDeleted(wi);
  ...
  auto RemoveSatisfiedLitsFromBinWatches = [&](TULit l) {
    ...
    TSpanTULit binWatches = TSpanTULit(
        m_W.get_ptr(wi.m_WBInd) + wi.GetLongEntries(), wi.m_BinaryWatches);

    auto itEndRemaining = remove_if(
        binWatches.begin(), binWatches.end(),
        [&](TULit otherLit) { return IsGloballySatisfied(otherLit); });
  ...
  auto HandleBinaryParents = [&]() {
    if (m_DecLevel > 0) {
      for (TUVar v = GetTrailNextVar(m_TrailLastVarPerDecLevel[0]);
           v != BadUVar; v = GetTrailNextVar(v)) {
        ...
        if (m_AssignmentInfo[v].m_IsAssignedInBinary) {
        ...
        } else if (m_VarInfo[v].m_ParentClsInd != BadClsInd &&
                   ClsChunkDeleted(m_VarInfo[v].m_ParentClsInd)) {
          ...
          assert(NV(2) || P("Var; parent cls: " + to_string(v) + " " +
                            HexStr(m_VarInfo[v].m_ParentClsInd) + ": " +
                            SLits(Cls(m_VarInfo[v].m_ParentClsInd)) + "\n"));
          if constexpr (Compress) {
  ...
  // Now we want to sift the variable indices (to the removed-by-now globally
  // assigned variables), if required One globally assigned variable will remain
  // in any case

  if (!siftVarIndices) {
    // Still need to HandleBinaryParents, since some non-binary parents could
    // have been turned binary
    HandleBinaryParents();
  ...
  if (m_Assumps.cap() > 0) {
    assert(all_of(m_Assumps.get_span_cap().begin(),
                  m_Assumps.get_span_cap().end(),
                  [&](TULit assumpLit) { return IsSatisfied(assumpLit); }));
  ...
  // Mark all the variables, whose watches (binary and cached lits in long)
  // contain the sifted indices
  for (TUVar vBefore = newLastExistingVar + 1;
       vBefore < m_HandyLitsClearBefore[0].cap(); ++vBefore) {
    ...
    for (uint8_t isNeg = 0; isNeg < 2; ++isNeg) {
      ...
      for (auto [currLongWatchInd, currLongWatchPtr] =
               make_pair((size_t)0, m_W.get_ptr(wi.m_WBInd));
           currLongWatchInd < wi.m_LongWatches;
  ...
  auto SiftLitsInWatches = [&](TULit l) {
    ...
    TSpanTULit binWatches = TSpanTULit(
        m_W.get_ptr(wi.m_WBInd) + wi.GetLongEntries(), wi.m_BinaryWatches);
    transform(binWatches.begin(), binWatches.end(), binWatches.begin(),
              [&](TULit secondLit) { return RetSiftedLit(secondLit); });
  ...
  if (!m_Assumps.cap() == 0) {
    ...
    transform(assumpSpan.begin(), assumpSpan.end(), assumpSpan.begin(),
              [&](TULit assumpLit) { return RetSiftedLit(assumpLit); });

    auto endUniquesIt = unique(assumpSpan.begin(), assumpSpan.end());
    m_Assumps.reserve_exactly(endUniquesIt - assumpSpan.begin());

    assert(all_of(assumpSpan.begin(), assumpSpan.end(), [&](TULit assumpLit) {
      return IsSatisfied(assumpLit) && IsAssump(assumpLit);
    }));
  }
  ...
  transform(e2iSpan.begin(), e2iSpan.end(), e2iSpan.begin(),
            [&](TULit l) { return RemovedLit2NewLit(l); });

  assert(NV(2) || P("E2I after: " + SE2I() + "\n"));

  // This function will shrink all the variable- and literal-dependent data
  // structures
  ...
  assert(NV(1) ||
         P("Finished renaming globally satisfied literals, but one\n"));
}
  ...
  for (TUInd clsInd = ClsLoopFirst(false); !ClsLoopCompleted();
    ...
    if (ClsChunkDeleted(clsInd)) {
      ...
      if constexpr (Compress) {
        ...
        if (bcHashInd.m_BitsForClsSize != 0 &&
            ba.bit_get(bcInd.m_BitStart, bcInd.m_BitsForClsSize) == 0) {
...
void CTopi<TLit, TUInd, Compress>::CompressWLs() {
  ...
  // and place log2(allocated) && l to be able to, respectively: (1) Skip to the
  // next chunk during compression; (2) Point from the watches to their updated
  // location in the watch-buffer
  ...
  for (TULit l = GetFirstLit(); l < GetNextLit(); ++l) {
    ...
    if (m_ParamCompressAllocatedPerWatch && wi.m_AllocatedEntries > 0 &&
        usedEntries == 0) {
      MarkWatchBufferChunkDeleted(wi);
    ...
    if (wi.m_AllocatedEntries > 0) {
      ...
      if (m_ParamCompressAllocatedPerWatch) {
        ...
        for (auto [nextEntry, remainingEntriesToMark] =
                 make_tuple(wi.m_WBInd + wi.m_AllocatedEntries,
                            allocatedEntriesBefore - wi.m_AllocatedEntries);
             remainingEntriesToMark != 0;
             remainingEntriesToMark -= bitFloor, nextEntry = WLEnd(nextEntry)) {
  ...
  m_W.template RemoveGarbage<TUInd>(
      LitsInPage, m_WNext, [&](TUInd wlInd) { return WLChunkDeleted(wlInd); },
      [&](TUInd wlInd) { return WLEnd(wlInd); },
...
void CTopi<TLit, TUInd, Compress>::CompressBuffersIfRequired() {
  ...
  if ((double)m_BWasted / nextBitOverall <= m_ParamWastedFractionThrToDelete ||
      IsUnrecoverable() || m_Status == TToporStatus::STATUS_USER_INTERRUPT) {
    return;
  }

  assert(NV(1) ||
         P("Compression started: wasted fraction is " +
           to_string((double)m_BWasted / (double)nextBitOverall) + " > " +
           to_string(m_ParamWastedFractionThrToDelete) + "\n"));
  assert(NV(2) || P("The trail: " + STrail() + "\n"));
  ...
  auto NotifyAboutRemainingChunkMove = [&](auto oldInd, auto newInd) {
    if (isAboveGlobalDecLevel) {
      auto UpdateParentIfRequired = [&](TULit l) {
        const TUVar v = GetVar(l);
        const bool isCurrClsParent = IsAssignedVar(v) &&
                                     !m_AssignmentInfo[v].IsAssignedBinary() &&
                                     m_VarInfo[v].m_ParentClsInd == oldInd;
        if (isCurrClsParent) {
  ...
  if constexpr (!Compress) {
    ...
    m_B.template RemoveGarbage<TUInd>(
        LitsInPage, m_BNext,
        [&](TUInd clsInd) { return ClsChunkDeleted(clsInd); },
        [&](TUInd clsInd) { return ClsEnd(clsInd); },
        NotifyAboutRemainingChunkMove);
  ...
  auto AddLongWatchLocal = [&](bool watchInd, TUInd clsInd) {
    ...
    if (IsFalsified(currLit) &&
        !(IsSatisfied(inlinedLit) &&
          GetAssignedDecLevel(inlinedLit) <= GetAssignedDecLevel(currLit))) {
      const auto clsFull = ConstClsSpan(clsInd);
      const auto currLitDecLevel = GetAssignedDecLevel(currLit);
      auto visitedLitIt = find_if(
          clsFull.begin() + 2, clsFull.end(), [&](const TULit visitedLit) {
            return IsSatisfied(visitedLit) &&
                   GetAssignedDecLevel(visitedLit) <= currLitDecLevel;
          });
  ...
  assert(NV(2) || P("Going over all the clauses after compression to "
                    "repopulate the watch lists\n"));
  for (TUInd clsInd = ClsLoopFirst(false); !ClsLoopCompleted();
       clsInd = ClsLoopNext()) {
    // assert(NV(2) || ClsChunkDeleted(clsInd) ? P("Chunk " + HexStr(clsInd) + "
    // was deleted!\n") : P("Clause " + HexStr((TUInd)clsInd) + ": " +
    // SLits(Cls(clsInd)) + "\n"));
    assert(!ClsChunkDeleted(clsInd));
    // Note that we cannot because of correctness (and also should not because
    // of efficiency) use the standard WLAddLongWatch procedure, since the
    // number of long watches doesn't change
