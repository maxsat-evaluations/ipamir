...
bool CTopi<TLit, TUInd, Compress>::DebugLongImplicationInvariantHolds(
  ...
  const auto highestNonWLDecLevel = GetAssignedDecLevel(
      *GetAssignedLitsHighestDecLevelIt((span<TULit>)cls, 2));
  assert(decLevel >= highestNonWLDecLevel);

  return parentClsIndIfLong != BadClsInd && l != BadULit && !IsAssigned(l) &&
         lOneOfFirstLits && IsFalsified(secondLit) &&
         decLevel == GetAssignedDecLevel(secondLit) &&
         decLevel >= highestNonWLDecLevel;
}

template <typename TLit, typename TUInd, bool Compress>
bool CTopi<TLit, TUInd, Compress>::Assign(TULit l, TUInd parentClsInd,
                                          TULit otherWatch, TUV decLevel,
                                          bool toPropagate,
                                          bool externalAssignment) {
  assert(parentClsInd == BadClsInd ||
         DebugLongImplicationInvariantHolds(l, decLevel, parentClsInd));
  ...
  if (unlikely(isAssigned)) {
    // Ignore if already assigned the same value; return false (contradiction),
    // if assigned the opposite value
    return isNegated;
  }
  ...
  // It is either that: (1) The trail is empty, or (2) The level is 0, or (3) A
  // new level is started, or (4) The level is non-empty So, there cannot be an
  // empty intermediate level
  assert(m_TrailEnd == BadUVar || decLevel == 0 || decLevel == m_DecLevel ||
         m_TrailLastVarPerDecLevel[decLevel] != BadUVar);

  TUVar trailPrev =
      m_TrailLastVarPerDecLevel[decLevel] == BadUVar && decLevel != 0
          ? m_TrailEnd
          : m_TrailLastVarPerDecLevel[decLevel];
  ...
  if (m_ParamPhaseBoostFlippedForced && IsForced(v) &&
      m_PolarityInfo[v].GetNextPolarityIsNegated() != IsNeg(l)) {
    UpdateScoreVar(v);
  }
  ...
  if (m_AssignmentInfo[v].m_IsAssump && IsAssumpFalsifiedGivenVar(v) &&
      (m_EarliestFalsifiedAssump == BadULit ||
       !IsAssigned(m_EarliestFalsifiedAssump) ||
       GetAssignedDecLevel(l) <=
           GetAssignedDecLevel(m_EarliestFalsifiedAssump))) {
    m_LatestEarliestFalsifiedAssump = m_EarliestFalsifiedAssump =
        GetAssumpLitForVar(v);
  ...
  if (m_CurrCustomBtStrat > 0 && m_VsidsHeap.var_score_exists(v)) {
    ...
    if (decLevel >= m_BestScorePerDecLevel.cap()) {
      ...
      ReserveExactly(m_BestScorePerDecLevel, GetNextVar(), 0,
                     "m_BestScorePerDecLevel in Assign");
    ...
    if (IsAssignedDecVar(v) || currScore > m_BestScorePerDecLevel[decLevel]) {
      ...
      assert(NV(2) ||
             P("m_BestScorePerDecLevel[" + to_string(decLevel) +
               "] updated to " + to_string(currScore) + " in Assign\n"));
    }
    assert(m_ParamAssertConsistency < 2 ||
           m_Stat.m_Conflicts < (uint64_t)m_ParamAssertConsistencyStartConf ||
           CalcMaxDecLevelScore(decLevel) == m_BestScorePerDecLevel[decLevel]);
  }
...
void CTopi<TLit, TUInd, Compress>::UnassignVar(TUVar v) {
  ...
  if (m_TrailLastVarPerDecLevel[lDecLevel] == v) {
    if (m_VarInfo[v].m_TrailPrev == BadUVar ||
        GetAssignedDecLevelVar(m_VarInfo[v].m_TrailPrev) != lDecLevel) {
...
void CTopi<TLit, TUInd, Compress>::FixPolarityInternal(TULit l, bool onlyOnce) {
  ...
  if (!m_PolarityInfoActivated || v >= m_PolarityInfo.cap()) {
    ...
    ReserveExactly(m_PolarityInfo, GetNextVar(), 0,
                   "m_PolarityInfo in FixPolarityInternal");
  }
  if (unlikely(IsUnrecoverable())) return;

  if (m_ParamUpdateParamsWhenVarFixed > 0 && !m_UpdateParamsWhenVarFixedDone &&
      ((onlyOnce && m_ParamUpdateParamsWhenVarFixed == 1) ||
       (!onlyOnce && m_ParamUpdateParamsWhenVarFixed == 2) ||
       m_ParamUpdateParamsWhenVarFixed == 3)) {
...
void CTopi<TLit, TUInd, Compress>::FixPolarity(TLit lExternal, bool onlyOnce) {
  ...
  if (m_DumpFile)
    (*m_DumpFile) << "lf " << lExternal << " " << (int)onlyOnce << endl;
...
void CTopi<TLit, TUInd, Compress>::ClearUserPolarityInfoInternal(TUVar v) {
  if (m_PolarityInfoActivated) {
    if (v >= m_PolarityInfo.cap()) {
      ReserveExactly(m_PolarityInfo, GetNextVar(), 0,
                     "m_PolarityInfo in ClearUserPolarityInfoInternal");
    }
...
bool CTopi<TLit, TUInd, Compress>::TrailAssertConsistency() {
  for (auto [nextIndStartDecLevel, v] = make_tuple(false, m_TrailStart);
       v != BadUVar; v = m_VarInfo[v].m_TrailNext) {
    [[maybe_unused]] const TULit l = GetAssignedLitForVar(v);
    [[maybe_unused]] const TUVar vTrailNext = m_VarInfo[v].m_TrailNext;

    [[maybe_unused]] const bool c1 =
        (vTrailNext == BadUVar && m_TrailEnd == v) ||
        (vTrailNext != BadUVar && m_VarInfo[vTrailNext].m_TrailPrev == v);
    assert(c1 || P("***** TrailAssertConsistency failure: " + STrail() + "\n"));
    assert(c1);

    [[maybe_unused]] const TUV vDecLevel = GetAssignedDecLevelVar(v);
    [[maybe_unused]] const TUV nextDecLevel =
        vTrailNext == BadUVar ? numeric_limits<TUV>::max()
                              : GetAssignedDecLevelVar(vTrailNext);

    [[maybe_unused]] const bool c2 =
        vDecLevel == nextDecLevel || m_TrailLastVarPerDecLevel[vDecLevel] == v;
    assert(c2 ||
           P("***** TrailAssertConsistency failure 2: " + STrail() + "\n"));
    assert(c2);
  }
  ...
  for (TUVar v = 1; v < GetNextVar(); ++v)
  {
          ...
          assert(!ai.m_Visit || find(vvSpan.begin(), vvSpan.end(), v) !=
  vvSpan.end()); assert(!ai.m_Root || find(rvSpan.begin(), rvSpan.end(), v) !=
  rvSpan.end());
  }
