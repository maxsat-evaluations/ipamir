...
class CVector : public CDynArray<T> {
 public:
  ...
  CVector(size_t initSz = 0, size_t next = 0)
      : CDynArray<T>(initSz), m_Next(next) {}
  CVector(size_t initSz, unsigned char initVal, size_t next)
      : CDynArray<T>(initSz, initVal), m_Next(next) {}
  // Copy constructor
  CVector(const CVector& v) : CDynArray<T>(v) { m_Next = v.m_Next; }
  // Assignment operator
  CVector& operator=(const CVector& v) {
    CDynArray<T>::operator=(v);
    m_Next = v.m_Next;
    return *this;
  }
  // Move constructor
  CVector(CVector&& v) : CDynArray<T>(move(v)) {
    m_Next = v.m_Next;
    v.m_Next = 0;
  }
  // Move assignment operator
  CVector& operator=(CVector&& v) {
    CDynArray<T>::operator=(move(v));
    m_Next = v.m_Next;
    v.m_Next = 0;
    return *this;
  }
  ...
  inline T&& pop_back() { return move(Par::m_B[--m_Next]); }

  inline T& back() { return Par::m_B[m_Next - 1]; }

  inline void clear() { m_Next = 0; }

  template <class TFuncApplyBeforeCleaning>
  inline void clear(TFuncApplyBeforeCleaning ApplyBeforeCleaning) {
    if (!empty()) {
      auto bSpan = get_span();
      for_each(bSpan.begin(), bSpan.end(),
               [&](T elem) { ApplyBeforeCleaning(elem); });
    }
    clear();
  }

  inline size_t size() const { return m_Next; }

  inline bool empty() const { return m_Next == 0; }
  ...
  inline operator span<T>() { return get_span(); }
