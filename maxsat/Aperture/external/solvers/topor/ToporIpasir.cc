...
class CToporIpasirWrapper : public CTopor<> {
 public:
  ...
  int Val(int lit) {
    ...
    switch (tlv) {
      case ApertureTopor::TToporLitVal::VAL_SATISFIED:
      case ApertureTopor::TToporLitVal::VAL_DONT_CARE:
        return lit;
      case ApertureTopor::TToporLitVal::VAL_UNSATISFIED:
        return -lit;
      default:
        return 0;
    }
  ...
  int Failed(int lit) { return IsAssumptionRequired(m_PrevAssump2Ind[lit]); }
...
const char* ipasir_signature() { return "IntelSatSolver"; }
...
void ipasir_set_parameter(void* solver, const char* paramName,
                          const char* paramVal) {
  CToporIpasirWrapper* tiw = reinterpret_cast<CToporIpasirWrapper*>(solver);
