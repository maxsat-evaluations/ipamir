...
/*
* Contagious bit-array data structures: unlike in the bit-array (see below) the LSB's go into the lower word, so the order is preserved, but for
* the price of an additional shift
*/
...
struct TCInfo
{
	...
	constexpr uint64_t W0CleanMask() const
	{
		return ~w0ReadMask;
	}

	constexpr uint64_t W1CleanMask() const
	{
		return ~w1ReadMask;
	}

	constexpr uint64_t ElemW0Mask() const
	{
		return w0ReadMask >> w0ReadShr;
	}

	constexpr uint8_t ElemW0Shl() const
	{
		return w0ReadShr;
	}

	constexpr uint64_t ElemW1Mask() const
	{
		return w1ReadMask << w1ReadShl;
	}

	constexpr uint64_t ElemW1Shr() const
	{
		return w1ReadShl;
	}
};

constexpr array<array<TCInfo, 64>, MAX_BITS_NUM> cMasksByBit = []
{
	array<array<TCInfo, 64>, MAX_BITS_NUM> byBitInfo = {};
	...
	for (uint8_t bitsInLit = 1; bitsInLit <= MAX_BITS_NUM; bitsInLit++)
	{
		for (uint8_t currBit = 0; currBit < MAX_BITS_NUM; ++currBit)
		{
			...
			{
				const uint8_t lsbBits = 64 - currBit;
				const uint8_t msbBits = bitsInLit - lsbBits;
				byBitInfo[bitsInLit - 1][currBit].w0ReadMask = ~LsbMask(currBit);
				byBitInfo[bitsInLit - 1][currBit].w0ReadShr = currBit;
				byBitInfo[bitsInLit - 1][currBit].w1ReadMask = LsbMask(msbBits);
				byBitInfo[bitsInLit - 1][currBit].w1ReadShl = lsbBits;
			}
			else
			{
				...
				byBitInfo[bitsInLit - 1][currBit].w1ReadShl = 0;
			}
		}
	}
	return byBitInfo;
...
// 1 bits starting from bit 0
static_assert(cMasksByBit[1 - 1][0].w0ReadMask == 1);
static_assert(cMasksByBit[1 - 1][0].w0ReadShr == 0);
static_assert(cMasksByBit[1 - 1][0].w1ReadMask == 0);
static_assert(cMasksByBit[1 - 1][0].w1ReadShl == 0);

// 1 bits starting from bit 4
static_assert(cMasksByBit[1 - 1][4].w0ReadMask == 16);
static_assert(cMasksByBit[1 - 1][4].w0ReadShr == 4);
static_assert(cMasksByBit[1 - 1][4].w1ReadMask == 0);
static_assert(cMasksByBit[1 - 1][4].w1ReadShl == 0);

// 1 bits starting from bit 63
static_assert(cMasksByBit[1 - 1][63].w0ReadMask == (uint64_t)1 << 63);
static_assert(cMasksByBit[1 - 1][63].w0ReadShr == 63);
static_assert(cMasksByBit[1 - 1][63].w1ReadMask == 0);
static_assert(cMasksByBit[1 - 1][63].w1ReadShl == 0);

// 2 bits starting from bit 0
static_assert(cMasksByBit[2 - 1][0].w0ReadMask == 3);
static_assert(cMasksByBit[2 - 1][0].w0ReadShr == 0);
static_assert(cMasksByBit[2 - 1][0].w1ReadMask == 0);
static_assert(cMasksByBit[2 - 1][0].w1ReadShl == 0);

// 2 bits starting from bit 4
static_assert(cMasksByBit[2 - 1][4].w0ReadMask == 16 + 32);
static_assert(cMasksByBit[2 - 1][4].w0ReadShr == 4);
static_assert(cMasksByBit[2 - 1][4].w1ReadMask == 0);
static_assert(cMasksByBit[2 - 1][4].w1ReadShl == 0);

// 2 bits starting from bit 63
static_assert(cMasksByBit[2 - 1][63].w0ReadMask == (uint64_t)1 << 63);
static_assert(cMasksByBit[2 - 1][63].w0ReadShr == 63);
static_assert(cMasksByBit[2 - 1][63].w1ReadMask == 1);
static_assert(cMasksByBit[2 - 1][63].w1ReadShl == 1);

// 3 bits starting from bit 0
static_assert(cMasksByBit[3 - 1][0].w0ReadMask == 7);
static_assert(cMasksByBit[3 - 1][0].w0ReadShr == 0);
static_assert(cMasksByBit[3 - 1][0].w1ReadMask == 0);
static_assert(cMasksByBit[3 - 1][0].w1ReadShl == 0);

// 3 bits starting from bit 4
static_assert(cMasksByBit[3 - 1][4].w0ReadMask == 16 + 32 + 64);
static_assert(cMasksByBit[3 - 1][4].w0ReadShr == 4);
static_assert(cMasksByBit[3 - 1][4].w1ReadMask == 0);
static_assert(cMasksByBit[3 - 1][4].w1ReadShl == 0);

// 3 bits starting from bit 61
static_assert(cMasksByBit[3 - 1][61].w0ReadMask == ((uint64_t)1 << 63) + ((uint64_t)1 << 62) + ((uint64_t)1 << 61));
static_assert(cMasksByBit[3 - 1][61].w0ReadShr == 61);
static_assert(cMasksByBit[3 - 1][61].w1ReadMask == 0);
static_assert(cMasksByBit[3 - 1][61].w1ReadShl == 0);

// 3 bits starting from bit 62
static_assert(cMasksByBit[3 - 1][62].w0ReadMask == ((uint64_t)1 << 63) + ((uint64_t)1 << 62));
static_assert(cMasksByBit[3 - 1][62].w0ReadShr == 62);
static_assert(cMasksByBit[3 - 1][62].w1ReadMask == 1);
static_assert(cMasksByBit[3 - 1][62].w1ReadShl == 2);

// 3 bits starting from bit 63
static_assert(cMasksByBit[3 - 1][63].w0ReadMask == ((uint64_t)1 << 63));
static_assert(cMasksByBit[3 - 1][63].w0ReadShr == 63);
static_assert(cMasksByBit[3 - 1][63].w1ReadMask == 3);
static_assert(cMasksByBit[3 - 1][63].w1ReadShl == 1);
