...
bool CTopi<TLit, TUInd, Compress>::Restart() {
  ...
  switch (m_CurrRestartStrat) {
    case RESTART_STRAT_NUMERIC:
      restartNow =
          m_DecLevel > 0 && m_ParamRestartNumericLocal
              ? m_ConfsSinceRestart -
                        m_RstNumericLocalConfsSinceRestartAtDecLevelCreation
                            [m_DecLevel] >=
                    m_RstNumericCurrConfThr
              : m_ConfsSinceRestart >= m_RstNumericCurrConfThr;
      if (restartNow) {
        if (m_ParamRestartNumericSubStrat == 1) {
          // Luby
          m_RstNumericCurrConfThr =
              (uint64_t)(RestartLubySequence(m_ParamRestartLubyConfIncr,
                                             m_Stat.m_Restarts) *
                         (double)m_ParamRestartNumericInitConfThr);
        } else {
          // Arithmetic
    ...
    case RESTART_STRAT_LBD:
      restartNow =
          m_RstGlueLbdWin.IsFullWindow() &&
          m_RstGlueLbdWin.GetAverage() * m_ParamRestartLbdAvrgMult >
              m_RstGlueGlobalLbdSum / (double)m_RstGlueAssertingGluedClss;
      if (restartNow) {
  ...
  if (unlikely(restartNow)) {
    ...
    if (currUnforceRestartsFraction != 0. &&
        currUnforceRestartsFraction != 1.) {
      const uint64_t restartInBlock =
          m_RestartsSinceInvStart % (uint64_t)m_ParamPhaseMngRestartsBlockSize;
      const double currRestartFractionCompleted =
          (double)restartInBlock / (double)m_ParamPhaseMngRestartsBlockSize;
      ...
      if (m_PhaseStage == m_PhaseInitStage) {
        if (m_PhaseStage == TPhaseStage::PHASE_STAGE_STANDARD &&
            currRestartFractionLeft <= currUnforceRestartsFraction) {
          m_PhaseStage = TPhaseStage::PHASE_STAGE_DONT_FORCE;
          assert(m_PhaseInitStage != m_PhaseStage);
          assert(NV(1) ||
                 P("Phase stage changed: " + GetPhaseStageStr() + "\n"));
        } else if (m_PhaseStage == TPhaseStage::PHASE_STAGE_DONT_FORCE &&
                   currRestartFractionCompleted >=
                       currUnforceRestartsFraction) {
...
void CTopi<TLit, TUInd, Compress>::RstNewAssertingGluedCls(TUV glue) {
  ...
  if (m_ParamRestartLbdBlockingEnable) {
    ...
    if (m_RstGlueAssertingGluedClss >
            m_ParamRestartLbdBlockingConfsToConsider &&
        m_RstGlueLbdWin.IsFullWindow() &&
        (double)m_AssignedVarsNum > m_ParamRestartLbdBlockingAvrgMult *
                                        m_RstGlueBlckAsgnWin.GetAverage()) {
      m_RstGlueLbdWin.Clear();
      ++m_Stat.m_RestartsBlocked;
    }
  }

  const auto glueThr = glue > m_ParamRestartLbdThresholdGlueVal
                           ? m_ParamRestartLbdThresholdGlueVal
                           : glue;
...
void CTopi<TLit, TUInd, Compress>::RestartInit() {
  ...
  auto InitGlue = [&]() {
    if (m_ParamRestartLbdBlockingEnable) {
      ...
      if (unlikely(m_RstGlueBlckAsgnWin.IsError()))
        SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                  "Couldn't reserve memory for m_RstGlueBlckAsgnWin");
    }
    m_RstGlueLbdWin.Init(m_ParamRestartLbdWinSize);
    if (unlikely(m_RstGlueLbdWin.IsError()))
      SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                "Couldn't reserve memory for m_RstGlueLbdWin");
  };
  ...
  if (m_QueryCurr == TQueryType::QUERY_INIT) {
  ...
  } else if (m_ParamRestartStrategyInit != m_ParamRestartStrategyS ||
             m_ParamRestartStrategyInit != m_ParamRestartStrategyN) {
    ...
    m_CurrRestartStrat = m_QueryCurr == TQueryType::QUERY_INC_SHORT
                             ? m_ParamRestartStrategyS
                             : m_ParamRestartStrategyN;

    if (m_CurrRestartStrat != prevRestartStrat) {
      assert(prevRestartStrat == RESTART_STRAT_LBD ||
             prevRestartStrat == RESTART_STRAT_NUMERIC);
      assert(m_CurrRestartStrat == RESTART_STRAT_LBD ||
             m_CurrRestartStrat == RESTART_STRAT_NUMERIC);

      if (m_CurrRestartStrat == RESTART_STRAT_NUMERIC) {
        // LBD --> NUMERIC
        assert(prevRestartStrat == RESTART_STRAT_LBD);
        m_RstNumericCurrConfThr =
            m_Stat.m_Conflicts + m_ParamRestartNumericInitConfThr;
        if (m_ParamRestartLbdBlockingEnable) {
  ...
  m_PhaseInitStage = m_PhaseStage =
      ...
      : m_ParamPhaseMngStartInvStrat == 0 ? TPhaseStage::PHASE_STAGE_STANDARD
      : m_ParamPhaseMngStartInvStrat == 1
          ? TPhaseStage::PHASE_STAGE_DONT_FORCE
          : ((double)rand() / (double)INT_MAX <= currUnforceRestartsFraction
                 ? TPhaseStage::PHASE_STAGE_DONT_FORCE
                 : TPhaseStage::PHASE_STAGE_STANDARD);
