...
{
	/*
	* Contagious bit-array: unlike in the bit-array (with the prefix BA) the LSB's go into the lower word, so the order is preserved, but there is a cost of an additional shift 
	*/

	static constexpr uint64_t CGetElem(const uint64_t* buf, uint64_t startBitNum, uint8_t elemWidth)
	{
		const auto& bitInfo = cMasksByBit[elemWidth - 1];
		...
		return ((buf[wordInd] & bitInfo[startBitInWord].w0ReadMask) >> bitInfo[startBitInWord].w0ReadShr) |
			((buf[wordInd + 1] & bitInfo[startBitInWord].w1ReadMask) << bitInfo[startBitInWord].w1ReadShl);
	}

	static constexpr uint64_t CGetElemAndGoToNext(const uint64_t* buf, uint64_t& bitNum, uint8_t elemWidth)
	{
		const auto res = CGetElem(buf, bitNum, elemWidth);
		bitNum += elemWidth;
		return res;
	}

	static constexpr void CSetElem(uint64_t* buf, uint64_t bitNum, uint8_t elemWidth, uint64_t elem)
	{
		const auto& bitInfo = cMasksByBit[elemWidth - 1];
		const size_t wordInd = (size_t)(bitNum >> 6);
		const size_t startBitInWord = (size_t)(bitNum & 63);

		buf[wordInd] &= bitInfo[startBitInWord].W0CleanMask();
		buf[wordInd] |= (elem & bitInfo[startBitInWord].ElemW0Mask()) << (uint64_t)bitInfo[startBitInWord].ElemW0Shl();
		buf[wordInd + 1] &= bitInfo[startBitInWord].W1CleanMask();
		buf[wordInd + 1] |= (elem & bitInfo[startBitInWord].ElemW1Mask()) >> (uint64_t)bitInfo[startBitInWord].ElemW1Shr();
	}
	...
	static constexpr void CSetElemAndGoToNext(uint64_t* buf, uint64_t& bitNum, uint8_t elemWidth, uint64_t elem)
	{
		CSetElem(buf, bitNum, elemWidth, elem);
		bitNum += elemWidth;
	}

	/*
	* Non-contagious bit-array: the LSB's go into the higher word, so the order is *not* preserved, but there is an additional shift saving
	* This implementation is preferable, when all the chunks have the same width
	*/
	...
	{
		const auto res = BAGetElem(buf, bitNum, elemWidth);
		bitNum += elemWidth;
		return res;
	}
	...
	static constexpr void BASetElem(uint64_t* buf, uint64_t elem)
	{
		...
		buf[0] &= bitInfo[bitNum].W0CleanMask();
		...
		else
		{
			...
			{
				buf[1] &= bitInfo[bitNum].W1CleanMask();
	...
	class CBitArraySingleElemWidth
	{
	public:
		CBitArraySingleElemWidth() : m_ElemWidth(ElemWidthConstExpr), m_ElemsNum(0), m_Buf(nullptr), m_BufAllocated(0)
		{
			static_assert(ElemWidthConstExpr <= 64);					
		}

		~CBitArraySingleElemWidth() 
		{ 
			free(m_Buf); 
		}
