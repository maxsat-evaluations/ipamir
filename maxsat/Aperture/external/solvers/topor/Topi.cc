...
void CTopi<TLit, TUInd, Compress>::ReadAnyParamsFromFile() {
  ...
  if (!tcf.is_open()) {
    SetStatus(TToporStatus::STATUS_PARAM_ERROR,
              "Cannot open the configuration parameter file " +
  ...
  auto Trim = [&](string_view s) {
    ...
    s.remove_suffix(
        min(s.size() - s.find_last_not_of(" \t\r\v\n") - 1, s.size()));
  ...
  while (getline(tcf, paramSetting)) {
    auto SetErrStatus = [&](const string& reason) {
      SetStatus(TToporStatus::STATUS_PARAM_ERROR,
                "Could parse line" + to_string(lineNum) + " : " + paramSetting +
                    " (after trimming \\t\\r\\v\\n); Reason: " + reason);
    };
...
template <typename TLit, typename TUInd, bool Compress>
CTopi<TLit, TUInd, Compress>::CTopi(TLit varNumHint)
    : m_InitVarNumAlloc(varNumHint <= 0 ? InitEntriesInB
                                        : (size_t)varNumHint + 1),
      m_E2ILitMap(m_InitVarNumAlloc, (size_t)0),
      m_HandleNewUserCls(m_InitVarNumAlloc),
      m_Watches(GetInitLitNumAlloc(), (size_t)0),
      m_TrailLastVarPerDecLevel(1, BadClsInd),
      m_VarInfo(m_InitVarNumAlloc, (size_t)0),
      m_Stat([&]() { return Compress ? m_BC.size() : 1; },
             [&]() { return Compress ? BCCapacitySum() : m_B.cap(); },
             [&]() { return Compress ? BCNextBitSum() / 64 + 1 : m_BNext; },
             [&]() { return GetMemoryLayout(); }, m_ParamVarActivityInc),
      m_VsidsHeap(m_Stat.m_VarActivityInc) {
  ...
  if (m_Params.IsError()) {
    ...
    SetStatus(
        TToporStatus::STATUS_ALLOC_FAILED,
        "CTopi<TLit,TUInd,Compress>::CTopi: couldn't allocate the main buffer");
  } else if (m_InitVarNumAlloc != 0 &&
             m_E2ILitMap.uninitialized_or_erroneous()) {
    SetStatus(
        TToporStatus::STATUS_ALLOC_FAILED,
        "CTopi<TLit,TUInd,Compress>::CTopi: couldn't allocate m_E2IVarMap");
  } else if (GetInitLitNumAlloc() != 0 &&
             m_Watches.uninitialized_or_erroneous()) {
    SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
              "CTopi<TLit,TUInd,Compress>::CTopi: couldn't allocate m_Watches");
  } else if (m_InitVarNumAlloc != 0 &&
             m_AssignmentInfo.uninitialized_or_erroneous()) {
    SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
              "CTopi<TLit,TUInd,Compress>::CTopi: couldn't allocate "
              "m_AssignmentInfo");
  } else if (m_InitVarNumAlloc != 0 && m_VarInfo.uninitialized_or_erroneous()) {
    SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
              "CTopi<TLit,TUInd,Compress>::CTopi: couldn't allocate m_VarInfo");
  } else if (m_TrailLastVarPerDecLevel.uninitialized_or_erroneous()) {
    SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
              "CTopi<TLit,TUInd,Compress>::CTopi: couldn't allocate "
              "m_TrailLastVarPerDecLevel");
  }
  ...
  // m_DebugModel = { false, true, false, true, false, false, false, true,
  // false, false, false, false, true, false, true, false, false, true, true,
  // true, true };
}

template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::SetParam(const string& paramName,
                                            double newVal) {
  ...
  auto HandleContextParam = [&](const string& contextParamPrefix,
                                vector<pair<string, double>>& paramVals) {
    if (paramName.starts_with(contextParamPrefix)) {
      paramVals.emplace_back(
          make_pair(paramName.substr(contextParamPrefix.size()), newVal));
      return true;
    }
    return false;
  };

  if (HandleContextParam(m_ContextParamAfterInitInvPrefix,
                         m_AfterInitInvParamVals)) {
    return;
  }

  if (HandleContextParam(m_ContextParamShortInvLifetimePrefix,
                         m_ShortInvLifetimeParamVals)) {
    return;
  }
...
TToporReturnVal CTopi<TLit, TUInd, Compress>::UnrecStatusToRetVal() const {
  ...
  if (unlikely(m_Status == TToporStatus::STATUS_CONTRADICTORY))
    return TToporReturnVal::RET_UNSAT;
  if (unlikely(m_Status == TToporStatus::STATUS_ALLOC_FAILED))
    return TToporReturnVal::RET_MEM_OUT;
  if (unlikely(m_Status == TToporStatus::STATUS_INDEX_TOO_NARROW))
    return TToporReturnVal::RET_INDEX_TOO_NARROW;
  if (unlikely(m_Status == TToporStatus::STATUS_PARAM_ERROR))
    return TToporReturnVal::RET_PARAM_ERROR;
  if (unlikely(m_Status == TToporStatus::STATUS_ASSUMPTION_REQUIRED_ERROR))
    return TToporReturnVal::RET_ASSUMPTION_REQUIRED_ERROR;
  if (unlikely(m_Status == TToporStatus::STATUS_GLOBAL_TIMEOUT))
    return TToporReturnVal::RET_TIMEOUT_GLOBAL;
  if (unlikely(m_Status == TToporStatus::STATUS_DRAT_FILE_PROBLEM))
    return TToporReturnVal::RET_DRAT_FILE_PROBLEM;
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::HandleIncomingUserVar(TLit v,
                                                         bool isUndoable) {
  if (unlikely((size_t)v >= m_E2ILitMap.cap())) {
    ...
    if (unlikely(m_E2ILitMap.uninitialized_or_erroneous())) {
      SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                "HandleIncomingUserVar: couldn't reserve m_E2IVarMap");
      return;
    }
  }
  ...
  if (m_E2ILitMap[v] == 0) {
    ...
    if (UseI2ELitMap()) {
      ...
      if (unlikely(m_I2ELitMap.uninitialized_or_erroneous())) {
        SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                  "HandleIncomingUserVar: couldn't reserve m_I2ELitMap");
        return;
      }
    ...
    if (!isUndoable) {
      ...
      if (unlikely(m_VsidsHeap.uninitialized_or_erroneous())) {
        SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                  "HandleIncomingUserVar: couldn't insert into m_VsidsHeap");
        return;
      }
    } else {
      ...
      if (unlikely(m_NewExternalVarsAddUserCls.uninitialized_or_erroneous())) {
        SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                  "HandleIncomingUserVar: couldn't push into "
                  "m_NewExternalVarsAddUserCls");
        return;
      }
    }
    ...
    if (unlikely(m_LastExistingVar >= m_AssignmentInfo.cap())) {
      ...
      if (unlikely(m_AssignmentInfo.uninitialized_or_erroneous())) {
        SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                  "HandleIncomingUserVar: couldn't realloc m_AssignmentInfo");
        return;
      }
    }
    ...
    if (unlikely(m_LastExistingVar >= m_VarInfo.cap())) {
      ...
      if (unlikely(m_VarInfo.uninitialized_or_erroneous())) {
        SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                  "HandleIncomingUserVar: couldn't realloc m_VarInfo");
        return;
      }
    }
  }
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::DumpSpan(const span<TLit> c,
                                            const string& prefix,
                                            const string& suffix,
                                            bool addNewLine) {
  (*m_DumpFile) << prefix;
...
void CTopi<TLit, TUInd, Compress>::AddUserClause(const span<TLit> c) {
  ...
  CApplyFuncOnExitFromScope<> onExit([&]() {
    if (isSuccess) {
      for (TLit ev : m_NewExternalVarsAddUserCls.get_span()) {
        ...
        if (unlikely(m_VsidsHeap.uninitialized_or_erroneous())) {
          SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                    "AddUserClause: couldn't insert into m_VsidsHeap");
          return;
        }
      }
      ...
      if (boostScores) {
        ...
        const double mult =
            InitClssBoostScoreStratIsClauseSizeAware()
                ? m_CurrInitClssBoostScoreMult * (1. / (double)(cls.size() - 1))
                : m_CurrInitClssBoostScoreMult;
        for (TULit l : cls) {
  ...
  if (unlikely(c.empty() || c[0] == 0)) {
    // An empty clause
    SetStatus(TToporStatus::STATUS_CONTRADICTORY,
              "AddClause: an empty clause provided");
    return;
  }

  // Storing the new clause while detecting tautologies and filtering out
  // duplicates
  ...
  for (const TLit l : c) {
    ...
    if constexpr (sizeof(TLit) == sizeof(size_t)) {
      if (unlikely(l == numeric_limits<TLit>::max() ||
                   l == numeric_limits<TLit>::min())) {
        // We cannot accommodate the last possible variable if  sizeof(TLit) ==
        // sizeof(size_t), since the allocation will fail
        SetStatus(
            TToporStatus::STATUS_INDEX_TOO_NARROW,
            "AddClause: we cannot accommodate the last possible variable if  "
            "sizeof(TLit) == sizeof(size_t), since allocation will fail");
        return;
      }
    }
    ...
    if (IsFalsified(litInternal) && GetAssignedDecLevel(litInternal) == 0) {
      // The literal is falsified at decision level 0: no need to add it to the
      // clause
      continue;
    }

    auto [isTau, isDuplicate, isAllocFailed] =
        m_HandleNewUserCls.NewLitIsTauIsDuplicate(litInternal);

    if (unlikely(isAllocFailed)) {
      SetStatus(
          TToporStatus::STATUS_ALLOC_FAILED,
          "AddClause: allocation failed during tautology&duplication test");
      return;
    }
    ...
    if (m_ParamAddClsRemoveClssGloballySatByLitMinSize !=
            numeric_limits<uint32_t>::max() &&
        c.size() > m_ParamAddClsRemoveClssGloballySatByLitMinSize &&
        IsSatisfied(litInternal) && GetAssignedDecLevel(litInternal) == 0) {
    ...
    if (unlikely(GetMaxLit(litInternal) >= m_Watches.cap())) {
      m_Watches.reserve_atleast((size_t)GetMaxLit(litInternal) + 1,
                                (size_t)BadULit);
      if (m_Watches.uninitialized_or_erroneous()) {
        SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                  "AddClause: couldn't reserve m_Watches");
        return;
      }
    }
  ...
  if (cls.size() == 0) {
    // The clause must have been contradictory (with some 0-level literals
    // removed)
    SetStatus(TToporStatus::STATUS_CONTRADICTORY,
              "AddClause: the clause is contradictory");
    return;
  }

  if (cls.size() == 1 && m_NewExternalVarsAddUserCls.size() == 1 &&
      m_TrailStart != BadUVar && GetAssignedDecLevelVar(m_TrailStart) == 0 &&
      (!IsAssigned(cls[0]) || GetAssignedDecLevel(cls[0]) != 0)) {
    ...
    const auto eSatLitIt = find_if(c.begin(), c.end(), [&](TLit l) {
      return ExternalLit2ExternalVar(l) == eSatVar;
    });
  ...
  assert(NV(2) || P("The user clause to internal clause: " + SUserLits(c) +
                    " to " + SLits(cls) + "\n"));
  ...
  auto OnContradiction = [&]() {
    ...
    m_ToPropagate.erase_if_may_reorder([&](TULit l) { return !IsAssigned(l); });
  };
  ...
  if (cls.size() == 1) {
    ...
    if (IsSatisfied(cls[0])) {
      ...
      // there is no need to run Simplify, since it doesn't satisfy or appear
      // falsified in any existing clauses, and this very function will make
      // sure that any future clauses satisfied by it are skipped and any
      // falsified appearances are skipped too
      if (GetVar(cls[0]) == m_LastExistingVar &&
          m_LastExistingVar != lastExistingVarStart &&
          m_LastGloballySatisfiedLitAfterSimplify ==
              m_VarInfo[GetVar(cls[0])].m_TrailPrev) {
  ...
  if (IsSatisfied(cls[0]) && IsFalsified(cls[1]) &&
...
bool CTopi<TLit, TUInd, Compress>::IsAssumptionRequired(size_t assumpInd) {
  ...
  if (m_Stat.m_SolveInvs == 0 || m_Status != TToporStatus::STATUS_UNSAT ||
      assumpInd >= m_UserAssumps.size()) {
    SetStatus(TToporStatus::STATUS_ASSUMPTION_REQUIRED_ERROR,
              m_Stat.m_SolveInvs == 0 ? "No Solve invocations so far"
              : m_Status != TToporStatus::STATUS_UNSAT
                  ? "The latest Solve didn't return TToporStatus::STATUS_UNSAT"
                  : "The assumption ID is beyond the number of assumptions in "
                    "the latest Solve invocation");
    return false;
  }

  if (m_LatestAssumpUnsatCoreSolveInvocation != m_Stat.m_SolveInvs) {
    assert(m_SelfContrOrGloballyUnsatAssumpSolveInv == m_Stat.m_SolveInvs ||
           m_LatestEarliestFalsifiedAssumpSolveInv == m_Stat.m_SolveInvs);
    assert(m_SelfContrOrGloballyUnsatAssumpSolveInv !=
           m_LatestEarliestFalsifiedAssumpSolveInv);
    assert(m_LatestEarliestFalsifiedAssumpSolveInv != m_Stat.m_SolveInvs ||
           IsAssigned(m_LatestEarliestFalsifiedAssump));

    // Making sure that we may return true only for one external variable out of
    // a group mapped to the same internal variable
  ...
  const bool nonDuplicatedAndVisited =
      m_UserAssumps[assumpInd] != 0 && IsVisited(E2I(m_UserAssumps[assumpInd]));
  const bool if0DecLevelThenFalsified =
      m_LatestEarliestFalsifiedAssumpSolveInv == m_Stat.m_SolveInvs ||
      !IsAssigned(m_SelfContrOrGloballyUnsatAssump) ||
      GetAssignedDecLevel(m_SelfContrOrGloballyUnsatAssump) != 0 ||
      IsFalsified(E2I(m_UserAssumps[assumpInd]));
  return nonDuplicatedAndVisited && if0DecLevelThenFalsified;
}

template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::AssignAssumptions(
    size_t firstUnassignedAssumpInd) {
  m_DecLevelOfLastAssignedAssumption = m_DecLevel;
  if (firstUnassignedAssumpInd != m_Assumps.cap()) {
    TSpanTULit potentiallyUnassignedAssumpsSpan =
        m_Assumps.get_span_cap(firstUnassignedAssumpInd);
    bool someAssumpsAreUnassigned = true;
    while (someAssumpsAreUnassigned) {
      someAssumpsAreUnassigned = false;
      for (size_t assumpLitI = 0;
           assumpLitI < potentiallyUnassignedAssumpsSpan.size(); ++assumpLitI) {
        const TULit assumpLit = potentiallyUnassignedAssumpsSpan[assumpLitI];

        if (!IsAssigned(assumpLit)) {
          NewDecLevel();

          [[maybe_unused]] bool isContraditory =
              Assign(assumpLit, BadULit, BadULit, m_DecLevel);
          ...
          if (m_EarliestFalsifiedAssump != BadULit) {
            if (IsAssigned(m_EarliestFalsifiedAssump) &&
                IsAssumpFalsifiedGivenVar(GetVar(m_EarliestFalsifiedAssump))) {
              SetStatus(TToporStatus::STATUS_UNSAT,
                        "Falsified assumption discovered after setting and "
                        "propagating an assumption at decision level " +
                            to_string(m_DecLevel));
              return;
            } else {
              // BCP backtracked
              size_t newFirstUnassignedAssumpInd =
                  FindFirstUnassignedAssumpIndex(firstUnassignedAssumpInd +
                                                 assumpLitI + 1);
    ...
    assert(all_of(potentiallyUnassignedAssumpsSpan.begin(),
                  potentiallyUnassignedAssumpsSpan.end(),
                  [&](TULit l) { return IsAssigned(l); }));
  }
}

template <typename TLit, typename TUInd, bool Compress>
size_t CTopi<TLit, TUInd, Compress>::FindFirstUnassignedAssumpIndex(
    size_t indexBeyondHandledAssumps) {
  ...
  for (size_t assumpI = indexBeyondHandledAssumps - 1;
       assumpI != (size_t)-1 && GetVar(m_Assumps[assumpI]) != prevDecVar;
       --assumpI) {
...
void CTopi<TLit, TUInd, Compress>::HandleAssumptionsIfBacktrackedBeyondThem() {
  ...
  size_t firstUnassignedAssumpInd =
      FindFirstUnassignedAssumpIndex(m_Assumps.cap());
  AssignAssumptions(firstUnassignedAssumpInd);
}

template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::HandleAssumptions(
    const span<TLit> userAssumps) {
  m_UserAssumps.assign(userAssumps.begin(), userAssumps.end());

  assert(NV(1) || P("Call " + to_string(m_Stat.m_SolveInvs) +
                    "; User assumptions: " + SUserLits(userAssumps) + "\n"));
  ...
  ReserveExactly(m_Assumps, userAssumps.size(),
                 "m_Assumps in HandleAssumptions");
  if (unlikely(IsUnrecoverable())) return;

  {
    TSpanTULit assumpsSpan = m_Assumps.get_span_cap();

    transform(userAssumps.begin(), userAssumps.end(), assumpsSpan.begin(),
              [&](TLit userLit) { return E2I(userLit); });
  }

  assert(NV(1) ||
         P("Call " + to_string(m_Stat.m_SolveInvs) +
           "; Intr assumptions: " + SLits(m_Assumps.get_span_cap()) + "\n"));
  ...
  for (size_t assumpI = 0; assumpI < m_Assumps.cap(); ++assumpI) {
    ...
    if (IsAssigned(lAssump) && GetAssignedDecLevel(lAssump) == 0) {
      if (IsFalsified(lAssump)) {
        ...
        assert(NV(1) || P("The external assumption " +
                          to_string(*find_if(
                              m_UserAssumps.begin(), m_UserAssumps.end(),
                              [&](TLit l) { return E2I(l) == lAssump; })) +
                          " is falsified at level 0\n"));
        m_SelfContrOrGloballyUnsatAssumpSolveInv = m_Stat.m_SolveInvs;
        SetStatus(TToporStatus::STATUS_UNSAT,
                  "An assumption is falsified at decision level 0");
        return;
      } else {
        RemoveAssump();
      }
      ...
      if (IsNeg(lAssump) != ai.m_IsAssumpNegated) {
        ...
        m_SelfContrOrGloballyUnsatAssumpSolveInv = m_Stat.m_SolveInvs;
        SetStatus(TToporStatus::STATUS_UNSAT,
                  "Discovered two assumption literals representing the same "
                  "variable in the two different polarities");
        return;
      } else {
        RemoveAssump();
      }
  ...
  // Find the first decision level, whose decision variable doesn't appear on
  // the list of assumptions
  TUV btLevel = 0;
  for (TUV dl = 1; dl <= m_DecLevel; ++dl, ++btLevel) {
    const TUVar currDecVar = GetDecVar(dl);
    assert(!m_AssignmentInfo[currDecVar].m_IsAssignedInBinary &&
           m_VarInfo[currDecVar].m_ParentClsInd == BadClsInd);
...
template <typename TLit, typename TUInd, bool Compress>
TToporReturnVal CTopi<TLit, TUInd, Compress>::Solve(
    const span<TLit> userAssumps, pair<double, bool> toInSecIsCpuTime,
    uint64_t confThr) {
  if (m_DumpFile) {
    // cout << "\tc ot <TimeOut> <IsCpuTimeOut>" << endl;
    (*m_DumpFile) << "ot " << toInSecIsCpuTime.first << " "
                  << (int)toInSecIsCpuTime.second << endl;
  ...
  m_QueryCurr = m_QueryCurr == TQueryType::QUERY_NONE ? TQueryType::QUERY_INIT
                : confThr <= (uint64_t)m_ParamShortQueryConfThrInv
                    ? TQueryType::QUERY_INC_SHORT
                    : TQueryType::QUERY_INC_NORMAL;

  const bool restoreParamsOnExit = m_QueryCurr == TQueryType::QUERY_INC_SHORT &&
                                   !m_ShortInvLifetimeParamVals.empty() &&
                                   !IsUnrecoverable();
  ...
  if (restoreParamsOnExit) {
    assert(m_QueryCurr == TQueryType::QUERY_INC_SHORT &&
           !m_ShortInvLifetimeParamVals.empty());

    paramsToRestore = new CTopiParams(m_Params);
    if (unlikely(paramsToRestore == nullptr)) {
      SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                "Solve: paramsToRestore allocation failed");
  ...
  if (m_ParamVerbosity > 0) {
    if (!m_AxePrinted) {
      PrintAxe();
      cout
          << print_as_color<ansi_color_code::blue>("c Literal bit-width= ")
          << print_as_color<ansi_color_code::blue>(to_string(sizeof(TLit) << 3))
          << print_as_color<ansi_color_code::blue>("; Clause-index bit-width= ")
          << print_as_color<ansi_color_code::blue>(
                 to_string(sizeof(TUInd) << 3))
          << print_as_color<ansi_color_code::blue>("; Compress = ")
          << print_as_color<ansi_color_code::blue>(Compress ? "1" : "0")
          << endl;
    }
    cout << m_Params.GetAllParamsCurrValues();
  }

  m_Stat.NewSolveInvocation(
      m_QueryCurr == CTopi<TLit, TUInd, Compress>::TQueryType::QUERY_INC_SHORT);
  ...
  CApplyFuncOnExitFromScope<> textDratAddCommentOnExit(
      m_OpenedDratFile != nullptr && !m_IsDratBinary, [&]() {
  ...
  CApplyFuncOnExitFromScope<> onExit([&]() {
    if (IsCbLearntOrDrat() &&
        (m_Status == TToporStatus::STATUS_UNSAT ||
         m_Status == TToporStatus::STATUS_CONTRADICTORY)) {
      vector<TULit> emptyCls;
      NewLearntClsApplyCbLearntDrat(emptyCls);
    }
    ...
    if (m_QueryCurr == TQueryType::QUERY_INIT &&
        m_ParamVarActivityUseMapleLevelBreakerAi !=
            m_ParamVarActivityUseMapleLevelBreaker) {
      m_ParamVarActivityUseMapleLevelBreaker =
          m_ParamVarActivityUseMapleLevelBreakerAi;
    }

    if (m_QueryCurr == TQueryType::QUERY_INIT &&
        m_ParamAddClsRemoveClssGloballySatByLitMinSizeAi !=
            m_ParamAddClsRemoveClssGloballySatByLitMinSize) {
      m_ParamAddClsRemoveClssGloballySatByLitMinSize =
          m_ParamAddClsRemoveClssGloballySatByLitMinSizeAi;
    }
  ...
  auto SetStatusLocalTimeout = [&]() {
    SetStatus(TToporStatus::STATUS_UNDECIDED,
              (string)(toInSecIsCpuTime.second ? "CPU" : "Wall") +
                  " timeout of " + to_string(toInSecIsCpuTime.first) +
                  " for the current Solve invocation reached");
  };
  auto SetStatusGlobalTimeout = [&]() {
    SetStatus(TToporStatus::STATUS_GLOBAL_TIMEOUT,
              "Global " + (string)(toInSecIsCpuTime.second ? "CPU" : "Wall") +
                  " timeout of " + to_string(toInSecIsCpuTime.first) +
                  " reached");
  };
  ...
  if (toInSecIsCpuTime.first < 1e100) {
    ...
    toInSecIsCpuTime.second
        ? m_Stat.m_TimeSinceLastSolveStart.SetModeCpuTime()
        : m_Stat.m_TimeSinceLastSolveStart.SetModeWallTime();
  ...
  // Make sure to create an internal variable for any new variables amongst the
  // assumptions (may increase m_Stat.m_MaxUserVar)
  ...
  ReserveVarAndLitData(userAssumps.size() -
                       (userAssumps.size() > 0 && userAssumps.back() == 0));
  ClsDeletionInit();
  RestartInit();
  DecisionInit();
  BacktrackingInit();
  ...
  if (pi.IsContradiction()) {
    if (m_DecLevel == 0) {
      SetStatus(TToporStatus::STATUS_CONTRADICTORY,
                "Global contradiction: discovered by BCP at decision level 0");
      ...
      if (m_EarliestFalsifiedAssump != BadULit) {
        SetStatus(TToporStatus::STATUS_UNSAT,
                  "Falsified assumption discovered after the initial BCP");
        return trv = TToporReturnVal::RET_UNSAT;
      }
    }
  }
  ...
  if (m_AssignedVarsNum == m_LastExistingVar) {
    ...
    SetStatus(TToporStatus::STATUS_SAT,
              "A model: discovered by the initial BCP before the search");
    return trv = TToporReturnVal::RET_SAT;
  }

  const auto confThrAfterThisConfNumReached =
      confThr == numeric_limits<decltype(confThr)>::max()
          ? numeric_limits<decltype(m_Stat.m_Conflicts)>::max()
          : m_Stat.m_Conflicts + confThr;
  ...
  m_DecLevelOfLastAssignedAssumption =
      m_Assumps.cap() == 0
          ? 0
          : GetAssignedDecLevel(
                *GetAssignedLitsHighestDecLevelIt(m_Assumps.get_span_cap(), 0));
  ...
  while (m_Status == TToporStatus::STATUS_UNDECIDED) {
    if (m_InterruptNow || (M_CbStopNow != nullptr &&
                           M_CbStopNow() == TStopTopor::VAL_STOP)) {
      if (m_InterruptNow) {
        SetStatus(TToporStatus::STATUS_USER_INTERRUPT,
                  "Interrupt by the Interrupt callback");
        m_InterruptNow = false;
      } else {
        SetStatus(TToporStatus::STATUS_USER_INTERRUPT,
                  "Interrupt by the StopNow callback");
      }
    }
    ...
    TULit l = Decide();
    assert(l != BadULit);
    assert(NV(2) || P("***** Decision at level " + to_string(m_DecLevel) +
                      ": " + SLit(l) + "\n"));
    ...
    [[maybe_unused]] bool isContraditory =
        Assign(l, BadClsInd, BadULit, m_DecLevel);
    assert(!isContraditory);
    assert(m_ParamAssertConsistency < 1 ||
           m_Stat.m_Conflicts < (uint64_t)m_ParamAssertConsistencyStartConf ||
           TrailAssertConsistency());
    ...
    TContradictionInfo contradictionInfo = BCP();
    const bool isContradictionBeforeConflictAnalysis =
        contradictionInfo.IsContradiction();

    ConflictAnalysisLoop(contradictionInfo);
    if (unlikely(IsUnrecoverable())) return trv = StatusToRetVal();

    // The earliest falsified assumption might be assigned&satisfied in the
    // corner case of a reimplication in BCP
    if (!contradictionInfo.IsContradiction() &&
        (m_EarliestFalsifiedAssump == BadULit ||
         !IsFalsified(m_EarliestFalsifiedAssump)) &&
    ...
    if (m_EarliestFalsifiedAssump != BadULit) {
      ...
      if (unlikely(!IsAssigned(m_EarliestFalsifiedAssump))) {
        SetStatus(
            TToporStatus::STATUS_ASSUMPTION_REQUIRED_ERROR,
            "Internal error: earliest falsified assumption must be assigned");
      } else {
        SetStatus(TToporStatus::STATUS_UNSAT, "Assumption flipped!");
      }
    } else if (!contradictionInfo.IsContradiction() &&
               m_AssignedVarsNum == m_LastExistingVar) {
      // All the variables have been assigned -> a model!
      SetStatus(TToporStatus::STATUS_SAT, "A model!");
    } else if (m_Stat.m_OverallTime.IsTimeoutSet() &&
               m_Stat.m_OverallTime.IsTimeout()) {
      // Global timeout
      SetStatusGlobalTimeout();
    } else if (m_Stat.m_Conflicts >= confThrAfterThisConfNumReached) {
      // Local conflict threshold reached
      SetStatus(TToporStatus::STATUS_UNDECIDED,
                "Conflicts threshold of " + to_string(confThr) + " reached");
      return trv = TToporReturnVal::RET_CONFLICT_OUT;
    } else if (m_Stat.m_TimeSinceLastSolveStart.IsTimeoutSet() &&
               m_Stat.m_TimeSinceLastSolveStart.IsTimeout()) {
      SetStatusLocalTimeout();
      return trv = TToporReturnVal::RET_TIMEOUT_LOCAL;
    }

    if (m_Status == TToporStatus::STATUS_UNDECIDED &&
        isContradictionBeforeConflictAnalysis && Restart()) {
      ...
      if (M_GetNextUnitClause != nullptr) {
        ...
        for (TLit eLit = M_GetNextUnitClause(m_ThreadId, true); eLit;
             eLit = M_GetNextUnitClause(m_ThreadId, false)) {
          TULit l = E2I(eLit);
          bool isContraditory = Assign(l, BadClsInd, BadULit, 0);
          if (isContraditory) {
            if (GetAssignedDecLevel(l) == 0) {
              assert(NV(1) || P("Global contradiction when assigning an "
                                "external unit clause\n"));
              return TToporReturnVal::RET_UNSAT;
            } else {
        ...
        if (isContradictionWithAssumptions) {
          assert(NV(1) || P("Contradiction with assumptions when assigning an "
                            "external unit clause\n"));
          return TToporReturnVal::RET_UNSAT;
        }
        ...
        if (m_AssignedVarsNum > assignedVarsNumBefore) {
          ...
          if (contradictionInfo.IsContradiction()) {
            assert(NV(1) ||
                   P("Contradiction when propagating external unit clauses\n"));
            return TToporReturnVal::RET_UNSAT;
          }
        }
      }
...
template <typename TLit, typename TUInd, bool Compress>
ApertureTopor::TToporLitVal CTopi<TLit, TUInd, Compress>::GetValue(
    TLit l) const {
  // This is in order not to crash when the user asks for a value beyond
  // m_MaxUserVar, which can legally happen (e.g., consider the input {4 0} {4 5
  // 6 7 0})
  ...
  // litInternal can be BadULit if the external variable doesn't appear on any
  // clauses
  return litInternal == BadULit     ? TToporLitVal::VAL_DONT_CARE
         : !IsAssigned(litInternal) ? TToporLitVal::VAL_UNASSIGNED
         : IsFalsified(litInternal) ? TToporLitVal::VAL_UNSATISFIED
                                    : TToporLitVal::VAL_SATISFIED;
}
...
template <typename TLit, typename TUInd, bool Compress>
std::string CTopi<TLit, TUInd, Compress>::GetStatStrShort(
    bool forcePrintingHead) {
...
template <typename TLit, typename TUInd, bool Compress>
uint64_t CTopi<TLit, TUInd, Compress>::GetAssumpReuseBacktrackLevelsSaved()
    const {
  return m_Stat.m_AssumpReuseBacktrackLevelsSaved;
}

template <typename TLit, typename TUInd, bool Compress>
vector<ApertureTopor::TToporLitVal> CTopi<TLit, TUInd, Compress>::GetModel()
    const {
  vector<ApertureTopor::TToporLitVal> m(m_E2ILitMap.cap());
...
template <typename TLit, typename TUInd, bool Compress>
ApertureTopor::TToporStatistics<TLit, TUInd>
CTopi<TLit, TUInd, Compress>::GetStatistics() const {
  return m_Stat;
}
...
void CTopi<TLit, TUInd, Compress>::SetOverallTimeoutIfAny() {
  if (m_ParamOverallTimeout != numeric_limits<double>::max()) {
    m_ParamOverallTimeoutIsCpu ? m_Stat.m_OverallTime.SetModeCpuTime()
                               : m_Stat.m_OverallTime.SetModeWallTime();
...
template <typename TLit, typename TUInd, bool Compress>
CTopi<TLit, TUInd, Compress>::TUV
CTopi<TLit, TUInd, Compress>::GetDecLevelWithBestScore(
    CTopi::TUV dlLowestIncl, CTopi::TUV dlHighestExcl) {
  ...
  const auto currDlSpan = m_BestScorePerDecLevel.get_const_span_cap(
      dlLowestIncl, dlHighestExcl - dlLowestIncl);
  ...
  for (TUV i = 0; i < (TUV)currDlSpan.size(); ++i) {
    ...
    if (currElem > maxElem ||
        (m_CurrCustomBtStrat == 1 && currElem == maxElem)) {
      maxElem = currElem;
      maxElemI = i;
    }
    assert(CalcMaxDecLevelScore(currDecLevel) ==
           m_BestScorePerDecLevel[currDecLevel]);
  }
...
void CTopi<TLit, TUInd, Compress>::NewDecLevel() {
  ...
  if (m_RstNumericLocalConfsSinceRestartAtDecLevelCreation.cap() != 0) {
    m_RstNumericLocalConfsSinceRestartAtDecLevelCreation[m_DecLevel] =
        m_ConfsSinceRestart;
  }
}
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::MarkWatchBufferChunkDeletedOrByLiteral(
    TUInd wlbInd, TUInd allocatedEntries, TULit l) {
...
bool CTopi<TLit, TUInd, Compress>::IsVisitedConsistent() const {
  for (TUVar v = 1; v < GetNextVar(); ++v) {
    ...
    const bool isInVisitedVec = find(m_VisitedVars.get_const_span().begin(),
                                     m_VisitedVars.get_const_span().end(),
                                     v) != m_VisitedVars.get_const_span().end();
...
double CTopi<TLit, TUInd, Compress>::CalcMaxDecLevelScore(TUV dl) const {
  double bestScore = 0;
  for (TUVar v = dl == 0 ? m_TrailStart : GetDecVar(dl);
       v != BadUVar && GetAssignedDecLevelVar(v) == dl;
       v = m_VarInfo[v].m_TrailNext) {
...
template <typename TLit, typename TUInd, bool Compress>
string ApertureTopor::CTopi<TLit, TUInd, Compress>::GetMemoryLayout() const {
  ...
  name2Mb["m_DecLevelsLastAppearenceCounter"] =
      m_DecLevelsLastAppearenceCounter.memMb();
  name2Mb["m_CurrClsCounters"] = m_CurrClsCounters.memMb();
  name2Mb["m_RstNumericLocalConfsSinceRestartAtDecLevelCreation"] =
      m_RstNumericLocalConfsSinceRestartAtDecLevelCreation.memMb();
  ...
  name2Mb["m_HandyLitsClearBefore"] =
      m_HandyLitsClearBefore[0].memMb() + m_HandyLitsClearBefore[1].memMb();
  ...
  name2Mb["m_TmpClss"] =
      accumulate(m_TmpClss.begin(), m_TmpClss.end(), (size_t)0,
                 [&](size_t sum, auto& it) { return sum + it.memMb(); });
  name2Mb["m_TmpClssDebug"] =
      accumulate(m_TmpClssDebug.begin(), m_TmpClssDebug.end(), (size_t)0,
                 [&](size_t sum, auto& it) { return sum + it.memMb(); });

  size_t overallSzInMb = 0;
  vector<pair<string, size_t>> name2MbVec(name2Mb.size());
  transform(name2Mb.begin(), name2Mb.end(), name2MbVec.begin(),
            [&](auto ssPair) {
              overallSzInMb += ssPair.second;
              return ssPair;
            });
  sort(name2MbVec.begin(), name2MbVec.end(),
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::SetParallelData(
    unsigned threadId,
...
// #topor: conflict clause analysis: 1) vivification: see Kissat paper; 2)
// replace glue with 2glue?
