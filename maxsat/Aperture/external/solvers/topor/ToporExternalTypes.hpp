...
enum class TToporStatus : uint8_t {
  // Status unknown
  STATUS_UNDECIDED,
  // The latest invocation had returned SAT and no clauses contradicting the
  // model were introduced since
  STATUS_SAT,
  // The latest invocation returned UNSAT, but this status might be temporary
  // (under assumptions)
  STATUS_UNSAT,
  ...
  // The instance is in memory-out state, meaning that one of the allocations
  // fails.
  ...
  // So-far: cannot accommodate the last possible variable if sizeof(TLit) ==
...
enum class TToporReturnVal : uint8_t {
  ...
  // Unsatisfiable
  RET_UNSAT,
  // Time-out for the last Solve invocation (the run-time is beyond
  // user-provided threshold value in Solve's parameter)
  RET_TIMEOUT_LOCAL,
  ...
  RET_USER_INTERRUPT,
  // Data doesn't fit into the buffer anymore; Try to compile and run Topor with
  // a wider TUInd, if you get this error
  RET_INDEX_TOO_NARROW,
  // Parameter-related error
  RET_PARAM_ERROR,
  // Error while processing assumption-required queries
  RET_ASSUMPTION_REQUIRED_ERROR,
  // Global time-out (the run-time is beyond user-provided threshold value in
  // /global/overall_timeout parameter: the solver is unusable, if this value is
  // returned)
  RET_TIMEOUT_GLOBAL,
...
// Callbacks return TStopTopor, which indicates, whether the solver should be
// stopped
enum class TStopTopor : bool { VAL_STOP, VAL_CONTINUE };
