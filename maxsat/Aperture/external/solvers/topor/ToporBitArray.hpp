...
class CBitArray : public CDynArray<uint64_t> {
 public:
  void bit_reserve_new_chunk(uint64_t newBitsRequired,
  ...
  void compress() { bit_resize_and_compress(m_NextBit); }
  ...
  template <class TConvertableToUint64>
  void bit_set_and_advance(TConvertableToUint64 elem, uint8_t elemWidth,
                           uint64_t& bitNum) {
  ...
  void copy_block(uint64_t bFrom, uint64_t bTo, uint64_t overallWidth) {
    ...
    if (GetRem(bFrom) == 0 && GetRem(bTo) == 0) {
      auto MemMove = [&](size_t outStartInd, size_t inpStartInd,
      ...
      MemMove((size_t)GetDiv(bTo), (size_t)GetDiv(bFrom),
              (size_t)GetDiv(overallWidth));
      const auto overallWidthRem = GetRem(overallWidth);
      if (overallWidthRem != 0) {
        copy(bFrom + overallWidth - overallWidthRem,
             bTo + overallWidth - overallWidthRem, (uint8_t)overallWidthRem);
      }
    } else {
      ...
      for (bFrom += bToRem, bTo += bToRem, overallWidth -= bToRem;
           overallWidth >= 64; overallWidth -= 64, bFrom += 64, bTo += 64) {
        copy(bFrom, bTo, 64);
      }
      if (overallWidth != 0) {
        copy(bFrom, bTo, (uint8_t)overallWidth);
 ...
 protected:
  ...
  constexpr size_t BitEntry2Uint64Entry(uint64_t bitEntry) {
    return (size_t)(bitEntry >> 6);
  }
};
};  // namespace ApertureTopor
