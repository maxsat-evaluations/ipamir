...
pair<bool, bool> CTopi<TLit, TUInd, Compress>::
    BCDeleteLitsByMovingToOtherBufferIfRequiredAssumingLastDeleted(
        TUInd& clsInd, TUV origSize, TUV lits2Remove,
        bool insertToSpareIfIteratorsBecomeInvalidated) {
  if (!BCDeleteLitsCausesMoveToAnotherBuffer(origSize, lits2Remove)) {
    return make_pair(false, false);
  }

  // This is the corner-case, when, after deleting the literals, the clause must
  // move to another compressed clause buffer
  auto ccs = ConstClsSpan(clsInd);
  bool spareUsed = false;
  const TBCInd newBcInd =
      BCCompress(ccs.subspan(0, origSize - lits2Remove), ClsGetIsLearnt(clsInd),
  ...
  // Counting the removed header and the rest of the removed literals as waste
  // (the removed literals themselves are supposed to be counted as waste by the
  // caller)
  m_BWasted += bcHashInd.GetFirstLitOffset() +
               bcHashInd.m_BitsForLit * (origSize - lits2Remove);

  // Updating clsInd to the new clause location
  clsInd = (TUInd)newBcInd;

  assert(NV(2) || insertToSpareIfIteratorsBecomeInvalidated ||
         P("\tBCDeleteLitsByMovingToOtherBufferIfRequiredAssumingLastDeleted, "
           "new clause: " +
           SLits(Cls(clsInd)) + "\n"));
...
template <typename TLit, typename TUInd, bool Compress>
CTopi<TLit, TUInd, Compress>::TBCInd CTopi<TLit, TUInd, Compress>::BCCompress(
    span<TULit> cls, bool isLearnt, TUV glue, bool stayForAnotherRound,
    float activity, bool insertToSpareIfIteratorsBecomeInvalidated,
    bool* spareUsedinCompressed) {
  const TBCHashId bcHashId(isLearnt, BCClsSize2Bits(cls.size()),
                           BCMaxLitWidth(cls));
  ...
  try {
    if (insertToSpareIfIteratorsBecomeInvalidated &&
      ...
      // Iterators are going to be invalidated, if the member is new (to test if
      // it is, test if the size will have been changed)!
    ...
    SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
              "BAddInitClause: couldn't insert into m_BC");
    return TBCInd();
  }

  CBitArray& ba = itExists.first->second;

  const uint64_t newBitsRequired =
      (uint64_t)bcHashId.m_BitsForClsSize +
      (uint64_t)cls.size() * (uint64_t)bcHashId.m_BitsForLit +
      (isLearnt ? bcHashId.GetBitsGlue() + 32 : 0);

  ba.bit_reserve_new_chunk(newBitsRequired);
  if (unlikely(ba.uninitialized_or_erroneous())) {
    SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
              "BCCompress: couldn't reserve new bit chunk");
    return TBCInd();
  }
  ...
  if (bcHashId.m_BitsForClsSize != 0) {
    ba.bit_push(BCClsSize2EncodedClsSize(cls.size()),
                bcHashId.m_BitsForClsSize);
    assert(ba.bit_get(bitStart, bcHashId.m_BitsForClsSize) ==
           BCClsSize2EncodedClsSize(cls.size()));
  }
  ...
  if (isLearnt) {
    ...
    assert(ba.bit_get(bitStart + bcHashId.m_BitsForClsSize,
                      bcHashId.GetBitsGlue()) == glue);
    // Push stay-for-another-round
    ba.bit_push(stayForAnotherRound, 1);
    assert(ba.bit_get(
               bitStart + bcHashId.m_BitsForClsSize + bcHashId.GetBitsGlue(),
               1) == (uint64_t)stayForAnotherRound);
    // Push the activity
    assert(activity >= 0.f);
    const uint32_t* uint32ActivityPtr = (uint32_t*)&activity;
    ba.bit_push(*uint32ActivityPtr, 31);
    assert(ba.bit_get(bitStart + bcHashId.m_BitsForClsSize +
                          bcHashId.GetBitsGlue() + 1,
                      31) == *uint32ActivityPtr);
  }

  // Push the literals
  for (const TULit l : cls) {
    ba.bit_push(l, bcHashId.m_BitsForLit);
    assert(ba.bit_get(ba.bit_get_next_bit() - bcHashId.m_BitsForLit,
                      bcHashId.m_BitsForLit) == l);
  }
  ...
  if (bci.IsError()) {
    stringstream ss;
    ss << "BCCompress: index too narrow -- bcHashId = " << hex << bcHashId
       << "; bitStart = " << dec << bitStart;
...
template <typename TLit, typename TUInd, bool Compress>
bool CTopi<TLit, TUInd, Compress>::BCIsIdenticalToDecompressed(
    TUInd clsInd, const TBCHashId currHashId, uint64_t bStart) {
  CBitArray& ba = m_BC.at(currHashId);
  const CCompressedCls cc(ba, TBCInd(currHashId, bStart));

  const bool isOversized =
      currHashId.m_IsLearnt && cc.size() > ClsLearntMaxSizeWithGlue;
...
template <typename TLit, typename TUInd, bool Compress>
void CTopi<TLit, TUInd, Compress>::BCDecompress(const TBCHashId currHashId,
                                                uint64_t& b) {
  [[maybe_unused]] const auto bStart = b;
  CBitArray& ba = m_BC[currHashId];
  const uint16_t encodedClsSize =
      (uint16_t)ba.bit_get_and_advance(b, currHashId.m_BitsForClsSize);
  const TUInd clsSize = BCEncodedClsSize2ClsSizeConst(
      encodedClsSize, currHashId.m_BitsForClsSize);

  const bool isOversized =
      currHashId.m_IsLearnt && clsSize > ClsLearntMaxSizeWithGlue;
  assert(!isOversized);

  const auto clsLitsStartOffset =
      EClsLitsStartOffset(currHashId.m_IsLearnt, isOversized);
  ...
  if (unlikely(newBNext < m_BNext)) {
    SetStatus(TToporStatus::STATUS_INDEX_TOO_NARROW,
              "BCExtractClause: too many literals in all the clauses combined");
    return;
  }
  ...
  if (unlikely(newBNext >= m_B.cap())) {
    ...
    if (unlikely(m_B.uninitialized_or_erroneous())) {
      SetStatus(TToporStatus::STATUS_ALLOC_FAILED,
                "BCExtractClause: couldn't reserve the buffer");
      return;
    }
  }
...
size_t CTopi<TLit, TUInd, Compress>::BCNextBitSum() const {
  const auto r = accumulate(m_BC.begin(), m_BC.end(), (size_t)0,
                            [&](size_t sum, auto& it) {
...
size_t CTopi<TLit, TUInd, Compress>::BCCapacitySum() const {
  const auto r = accumulate(
      m_BC.begin(), m_BC.end(), (size_t)0,
      [&](size_t sum, auto& it) { return sum + (size_t)it.second.cap(); });
