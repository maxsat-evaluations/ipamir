...
// Stores a window of the last N values, including a possibility to get an
// average Used for Glucose-inspired restart strategies
template <typename TUV>
class TWinAverage {
 public:
  TWinAverage() {}
  inline void Init(uint16_t maxCap) { m_DynArray.reserve_exactly(maxCap, 0); }
  ...
  inline double GetAverage() const {
    return m_Sum / (double)(IsFullWindow() ? MaxCap() : m_NextInd);
  }
  ...
  inline bool IsError() const {
    return m_DynArray.uninitialized_or_erroneous();
  }
