...
CTopor<TLit, TUInd, Compress>::CTopor(TLit varsNumHint)
    : m_Topi(new CTopi<TLit, TUInd, Compress>(varsNumHint)) {
  ...
  // TUInd must be at least as wide as TLit (since the clause and watch buffers
  // will store literals, amongst others)
...
template <typename TLit, typename TUInd, bool Compress>
TToporReturnVal CTopor<TLit, TUInd, Compress>::Solve(
    const span<TLit> assumps, pair<double, bool> toInSecIsCpuTime,
    uint64_t confThr) {
...
template <typename TLit, typename TUInd, bool Compress>
void CTopor<TLit, TUInd, Compress>::SetParam(const string& paramName,
                                             double newVal) {
  m_Topi->SetParam(paramName, newVal);
}

template <typename TLit, typename TUInd, bool Compress>
ApertureTopor::TToporLitVal CTopor<TLit, TUInd, Compress>::GetLitValue(
    TLit l) const {
...
template <typename TLit, typename TUInd, bool Compress>
std::string CTopor<TLit, TUInd, Compress>::GetStatStrShort(
...
template <typename TLit, typename TUInd, bool Compress>
uint64_t CTopor<TLit, TUInd, Compress>::GetAssumpReuseBacktrackLevelsSaved()
    const {
...
template <typename TLit, typename TUInd, bool Compress>
void CTopor<TLit, TUInd, Compress>::DumpDrat(ofstream& openedDratFile,
                                             bool isDratBinary,
...
template <typename TLit, typename TUInd, bool Compress>
void CTopor<TLit, TUInd, Compress>::SetCbNewLearntCls(
    TCbNewLearntCls<TLit> CbNewLearntCls) {
...
template <typename TLit, typename TUInd, bool Compress>
void CTopor<TLit, TUInd, Compress>::SetParallelData(
    unsigned threadId,
...
std::ostream& operator<<(std::ostream& os, const TToporReturnVal& trv) {
  switch (trv) {
    ...
    case TToporReturnVal::RET_UNSAT:
      os << "UNSAT";
      break;
    case TToporReturnVal::RET_TIMEOUT_LOCAL:
      os << "TIMEOUT_LOCAL";
      break;
    case TToporReturnVal::RET_CONFLICT_OUT:
      os << "CONFLICT_OUT";
      break;
    case TToporReturnVal::RET_MEM_OUT:
      os << "MEM_OUT";
      break;
    case TToporReturnVal::RET_USER_INTERRUPT:
      os << "USER_INTERRUPT";
      break;
    ...
    case TToporReturnVal::RET_ASSUMPTION_REQUIRED_ERROR:
      os << "ASSUMPTION_REQUIRED_ERROR";
      break;
    case TToporReturnVal::RET_TIMEOUT_GLOBAL:
      os << "TIMEOUT_GLOBAL";
      break;
    case TToporReturnVal::RET_DRAT_FILE_PROBLEM:
      os << "DRAT_FILE_PROBLEM";
      break;
    case TToporReturnVal::RET_EXOTIC_ERROR:
      os << "EXOTIC_ERROR";
      break;
    default:
      os << "TToporReturnVal : undefined value!";
  }
  return os;
}
}  // namespace ApertureTopor

template class ApertureTopor::CTopor<int32_t, uint32_t, false>;
template class ApertureTopor::CTopor<int32_t, uint64_t, false>;
template class ApertureTopor::CTopor<int32_t, uint64_t, true>;
