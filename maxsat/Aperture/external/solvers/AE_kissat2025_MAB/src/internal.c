...
void kissat_release (kissat *solver) {
  ...
  // ~Aperture
  kissat_release_polarities (solver);
...
void kissat_set_polarity (kissat *solver, int elit) {
  kissat_require_initialized (solver);
  kissat_require_valid_external_internal (elit);

  unsigned ilit = kissat_import_literal (solver, elit);
  unsigned idx = IDX (ilit);
  solver->polarities[idx] = elit > 0 ? 1 : -1;
}

void kissat_clear_polarity (kissat *solver, int elit) {
  kissat_require_initialized (solver);
  kissat_require_valid_external_internal (elit);

  unsigned ilit = kissat_import_literal (solver, elit);
  unsigned idx = IDX (ilit);
  solver->polarities[idx] = 0;
}

void kissat_freeze_variable (kissat *solver, int elit) {
  kissat_require_initialized (solver);
  kissat_require_valid_external_internal (elit);

  unsigned ilit = kissat_import_literal (solver, elit);
  unsigned idx = IDX (ilit);
  FLAGS (idx)->frozen = true;
  FLAGS (idx)->eliminate = false;
}

void kissat_melt_variable (kissat *solver, int elit) {
  kissat_require_initialized (solver);
  kissat_require_valid_external_internal (elit);

  unsigned ilit = kissat_import_literal (solver, elit);
  unsigned idx = IDX (ilit);
  FLAGS (idx)->frozen = false;
}