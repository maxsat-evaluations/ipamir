...
static int forward_false_satisfiable (struct kissat *solver) {
  ...
  for (all_stack (import, import, solver->import)) {
    ...
    if (VALUE (lit))
      continue;
    // Polarity-aware: prefer polarity direction if set, otherwise default false
    const unsigned not_lit = NOT (lit);
    unsigned decision, fallback;
    if (solver->polarities && solver->polarities[idx] > 0) {
      decision = lit;
      fallback = not_lit;
    } else if (solver->polarities && solver->polarities[idx] < 0) {
      decision = not_lit;
      fallback = lit;
    } else {
      decision = not_lit;  // default: false
      fallback = lit;
    }
    kissat_internal_assume (solver, decision);
    ...
    if (solver->level > 1) {
      ...
      kissat_internal_assume (solver, fallback);
...
int kissat_lucky (struct kissat *solver) {
  ...
  // Check if any polarities are set
  bool has_positive_polarity = false;
  bool has_negative_polarity = false;
  if (solver->polarities) {
    for (all_variables (idx)) {
      if (!ACTIVE (idx))
        continue;
      if (solver->polarities[idx] > 0)
        has_positive_polarity = true;
      if (solver->polarities[idx] < 0)
        has_negative_polarity = true;
      if (has_positive_polarity && has_negative_polarity)
        break;
    }
  }

  // no_all_negative_clauses assigns everything TRUE,
  // skip if any polarity is set to FALSE (-1)
  if (!has_negative_polarity && no_all_negative_clauses (solver)) {
    for (all_variables (idx)) {
      if (!ACTIVE (idx))
        continue;
      const unsigned lit = LIT (idx);
  ...
  // no_all_positive_clauses assigns everything FALSE,
  // skip if any polarity is set to TRUE (+1)
  if (!res && !has_positive_polarity && no_all_positive_clauses (solver)) {
    for (all_variables (idx)) {
      if (!ACTIVE (idx))
        continue;
      const unsigned lit = LIT (idx);
