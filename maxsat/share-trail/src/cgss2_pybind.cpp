  ...
  for (Py_ssize_t i = 0; i < n; ++i) {
    ...
    if (PyErr_Occurred()) {
      ...
      PyErr_SetString(PyExc_TypeError,
                      "Clause/assumption literals must be integers");
      return -1;
    }
...
static int validate_kwargs(PyObject *kwds) {
  ...
  static const std::set<std::string> valid_keys = {
      ...
      "abst_cg",  "hardening",
      "minimize", "trim",
      "exhaust",  "greedy_costs",
      "use_ub",   "relax_threshold"};
...
static int PyCGSS2_init(PyCGSS2Object *self, PyObject *args, PyObject *kwds) {
  if (validate_kwargs(kwds) < 0)
    return -1;

  PyObject *wcnf = nullptr;
  if (!PyArg_ParseTuple(args, "O", &wcnf)) {
    PyErr_SetString(PyExc_TypeError,
                    "CGSS2 expects one positional argument: wcnf");
    return -1;
  }
  ...
  op.prepare(std::cerr);
  if (op.strs["satsolver"] == "")
    op.strs["satsolver"] = "glucose41";

  int verbose = 0;
  if (get_kw_int(kwds, "verbose", &verbose, 0) < 0)
    return -1;
  ...
  if (get_kw_bool(kwds, "am1s", &btmp, self->solver->am1s) < 0)
    return -1;
  self->solver->am1s = btmp;
  if (get_kw_bool(kwds, "strat", &btmp, self->solver->strat) < 0)
    return -1;
  self->solver->strat = btmp;
  if (get_kw_bool(kwds, "blo", &btmp, self->solver->blo) < 0)
    return -1;
  self->solver->blo = btmp;
  if (get_kw_bool(kwds, "wce", &btmp, self->solver->WCE) < 0)
    return -1;
  self->solver->WCE = btmp;
  if (get_kw_bool(kwds, "fc", &btmp, self->solver->FC) < 0)
    return -1;
  self->solver->FC = btmp;
  if (get_kw_int(kwds, "abst_cg", &itmp, self->solver->AbstCG) < 0)
    return -1;
  self->solver->AbstCG = itmp;
  if (get_kw_int(kwds, "hardening", &itmp, self->solver->hardening) < 0)
    return -1;
  self->solver->hardening = itmp;
  if (get_kw_int(kwds, "minimize", &itmp, self->solver->minimize) < 0)
    return -1;
  self->solver->minimize = itmp;
  if (get_kw_int(kwds, "trim", &itmp, self->solver->trim) < 0)
    return -1;
  self->solver->trim = itmp;
  if (get_kw_bool(kwds, "exhaust", &btmp, self->solver->exhaust) < 0)
    return -1;
  self->solver->exhaust = btmp;
  if (get_kw_bool(kwds, "greedy_costs", &btmp, self->solver->greedy_costs) < 0)
    return -1;
  self->solver->greedy_costs = btmp;
  if (get_kw_bool(kwds, "use_ub", &btmp, self->solver->use_ub) < 0)
    return -1;
  self->solver->use_ub = btmp;
  if (get_kw_int(kwds, "relax_threshold", &itmp,
                 self->solver->core_relax_threshold) < 0)
    return -1;
  ...
  if (!hard || !soft || !wght) {
    ...
    PyErr_SetString(PyExc_AttributeError,
                    "wcnf must expose attributes: hard, soft, wght");
    return -1;
  }
  ...
  if (ssz != wsz) {
    ...
    PyErr_SetString(PyExc_ValueError,
                    "wcnf.soft and wcnf.wght must have equal length");
    return -1;
  }

  std::vector<int> clause;
  for (Py_ssize_t i = 0; i < hsz; ++i) {
    if (pyseq_to_int_vector(PySequence_Fast_GET_ITEM(hard_seq, i), clause) <
        0) {
      Py_DECREF(hard_seq);
      Py_DECREF(soft_seq);
      Py_DECREF(wght_seq);
  ...
  for (Py_ssize_t i = 0; i < ssz; ++i) {
    if (pyseq_to_int_vector(PySequence_Fast_GET_ITEM(soft_seq, i), clause) <
        0) {
      Py_DECREF(hard_seq);
      Py_DECREF(soft_seq);
      Py_DECREF(wght_seq);
    ...
    unsigned long long w =
        PyLong_AsUnsignedLongLong(PySequence_Fast_GET_ITEM(wght_seq, i));
    if (PyErr_Occurred()) {
      Py_DECREF(hard_seq);
      Py_DECREF(soft_seq);
      Py_DECREF(wght_seq);
      PyErr_SetString(PyExc_TypeError,
                      "wcnf.wght must contain non-negative integers");
      return -1;
    }
...
static PyObject *PyCGSS2_solve(PyCGSS2Object *self, PyObject *args,
                               PyObject *kwargs) {
  static const char *kwlist[] = {"assumptions", nullptr};
  PyObject *assumptions = Py_None;
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|O",
                                   const_cast<char **>(kwlist), &assumptions)) {
    return nullptr;
  }

  if (assumptions != Py_None) {
    PyObject *seq = PySequence_Fast(
        assumptions, "assumptions must be a sequence of integers or None");
    if (!seq)
      return nullptr;
    Py_ssize_t n = PySequence_Fast_GET_SIZE(seq);
    PyObject **items = PySequence_Fast_ITEMS(seq);
...
static PyObject *PyCGSS2_set_soft(PyCGSS2Object *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "iK", &lit, &weight))
    return nullptr;
  self->solver->ipamir_add_soft_lit(-lit, static_cast<uint64_t>(weight));
  Py_RETURN_NONE;
}

static PyObject *PyCGSS2_add_clause(PyCGSS2Object *self, PyObject *args,
                                    PyObject *kwargs) {
  static const char *kwlist[] = {"clause", "weight", nullptr};
  PyObject *clause_obj = nullptr;
  PyObject *weight_obj = Py_None;
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|O",
                                   const_cast<char **>(kwlist), &clause_obj,
                                   &weight_obj)) {
    return nullptr;
  }

  std::vector<int> clause;
  if (pyseq_to_int_vector(clause_obj, clause) < 0)
    return nullptr;
  ...
  if (weight_obj != Py_None) {
    ...
    if (PyErr_Occurred()) {
      PyErr_SetString(PyExc_TypeError,
                      "weight must be a non-negative integer or None");
      return nullptr;
    }
...
static PyMethodDef PyCGSS2_methods[] = {
    {"solve", reinterpret_cast<PyCFunction>(PyCGSS2_solve),
     METH_VARARGS | METH_KEYWORDS, "Solve with optional assumptions"},
    {"get_model", reinterpret_cast<PyCFunction>(PyCGSS2_get_model), METH_NOARGS,
     "Get model"},
    {"get_cost", reinterpret_cast<PyCFunction>(PyCGSS2_get_cost), METH_NOARGS,
     "Get objective cost"},
    {"set_soft", reinterpret_cast<PyCFunction>(PyCGSS2_set_soft), METH_VARARGS,
     "Set soft literal weight"},
    {"add_clause", reinterpret_cast<PyCFunction>(PyCGSS2_add_clause),
     METH_VARARGS | METH_KEYWORDS, "Add hard/soft clause"},
    {nullptr, nullptr, 0, nullptr}};

static PyTypeObject PyCGSS2Type = {
    PyVarObject_HEAD_INIT(nullptr, 0).tp_name = "cgss2_native.CGSS2",
...
static PyModuleDef cgss2_native_module = {PyModuleDef_HEAD_INIT,
                                          "cgss2_native",
...
PyMODINIT_FUNC PyInit_cgss2_native(void) {
  if (PyType_Ready(&PyCGSS2Type) < 0)
    return nullptr;
  PyObject *m = PyModule_Create(&cgss2_native_module);
  if (!m)
    return nullptr;
  Py_INCREF(&PyCGSS2Type);
  if (PyModule_AddObject(m, "CGSS2",
                         reinterpret_cast<PyObject *>(&PyCGSS2Type)) < 0) {
