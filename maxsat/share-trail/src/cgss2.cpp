...
void CGSS2::reserve_vars(int nof_vars) {
  ...
  connsn.resize(n);
  connsp.resize(n);
  lit_to_consequences.resize(n);
  ordered_graph.reserve_vars(nof_vars);
}
...
void CGSS2::add_weight(int l, int64_t delta) {
  if (delta == 0) return;
  ...
  if (delta < 0) {
    if (delta == std::numeric_limits<int64_t>::min()) {
      log.l(0) << "c bug: add_weight got INT64_MIN delta"
               << " lit=" << l
               << " ws=" << ws[l]
               << "\n";
      exit(-1);
    ...
    if (old < dec) {
      log.l(0) << "c bug: add_weight underflow"
               << " lit=" << l
               << " ws=" << old
               << " delta=" << delta
               << "\n";
    ...
    if (std::numeric_limits<uint64_t>::max() - old < inc) {
      log.l(0) << "c bug: add_weight overflow"
               << " lit=" << l
               << " ws=" << old
               << " delta=" << delta
               << "\n";
...
void CGSS2::set_weight_raw(int l, uint64_t nw) {
  reserve_vars(litVariable(l));
  ...
  if (old < climit) {
    if (sumw < old) {
      log.l(0) << "c bug: sumw underflow in set_weight_raw"
               << " lit=" << l
               << " old=" << old
               << " sumw=" << sumw
               << "\n";
  ...
  if (nw != 0 && nw != HARDWEIGHT && !(bit_data[l] & WEIGHTED)) {
    bit_data[l] |= WEIGHTED;
...
void CGSS2::init_weight(int l, uint64_t w) {
  if ((w || wi[l]) && log.level(5)) {
    log.l(5) << "c init_weight " << l << " " << w << "\n";
  }
...
void CGSS2::sync_unit_soft_clause_weight(int lit, uint64_t weight) {
  bool seen = false;

  for (unsigned i = 0; i < soft_clauses.size(); ++i) {
    if (soft_clauses[i].size() != 1) continue;
    if (soft_clauses[i][0] != lit) continue;

    if (!seen) {
      soft_clause_weight[i] = weight;
      seen = true;
    } else {
      soft_clause_weight[i] = 0;
    }
  }

}

void CGSS2::queue_weight_update(int lit, uint64_t weight) {
  reserve_vars(litVariable(lit));

  pending_weight_updates.push_back({lit, weight});
  pending_weight_changed_lits.push_back(lit);
}

void CGSS2::restore_incremental_assumption_weights(
    const std::vector<int>& previous_user_assumptions,
    const std::vector<int>& current_user_assumptions
) {
  std::vector<int> prev = previous_user_assumptions;
  std::vector<int> curr = current_user_assumptions;

  normalize_lit_vector(prev);
  normalize_lit_vector(curr);
  ...
  if (last_hardening_lit) {
    set_weight(last_hardening_lit, wi[last_hardening_lit]);
  }
}

void CGSS2::coalesce_pending_weight_updates() {
  if (pending_weight_updates.empty()) {
    pending_weight_changed_lits.clear();
    return;
  }

  std::map<int, uint64_t> last_update;

  for (const auto& p : pending_weight_updates) {
    last_update[p.first] = p.second;
  }

  pending_weight_updates.clear();
  pending_weight_changed_lits.clear();

  for (const auto& p : last_update) {
    int lit = p.first;
    ...
    pending_weight_updates.push_back(p);
    pending_weight_changed_lits.push_back(lit);
  }
}

void CGSS2::prepare_incremental_query_assumptions(
    const std::vector<int>& current_user_assumptions
) {
  hardening_lit = posLit(new_var());
  last_hardening_lit = hardening_lit;

  base_assumptions = current_user_assumptions;
  normalize_lit_vector(base_assumptions);
  ...
  UB = numeric_limits<uint64_t>::max();

  cores_to_relax.clear();
  cores_to_relax_fc.clear();
}

void CGSS2::apply_pending_weight_updates() {
  for (const auto& p : pending_weight_updates) {
    int lit = p.first;
    uint64_t weight = p.second;

    apply_lit_weight_update_now(lit, weight);
  }

  pending_weight_updates.clear();
}

void CGSS2::normalize_lit_vector(std::vector<int>& xs) const {
  std::sort(xs.begin(), xs.end());
  xs.erase(std::unique(xs.begin(), xs.end()), xs.end());
}

void CGSS2::compute_incremental_diff_seed(const std::vector<int>& current_user_assumptions_) {
  std::vector<int> current_user_assumptions = current_user_assumptions_;
  normalize_lit_vector(current_user_assumptions);

  std::vector<int> previous_user_assumptions = last_user_assumptions;
  normalize_lit_vector(previous_user_assumptions);

  current_diff_seed.clear();

  for (int l : pending_weight_changed_lits) {
    current_diff_seed.push_back(l);
    current_diff_seed.push_back(litNegation(l));
  }

  {
    ...
    while (i < previous_user_assumptions.size()) {
      int removed = 0;

      if (j >= current_user_assumptions.size()) {
        removed = previous_user_assumptions[i++];
      } else if (previous_user_assumptions[i] < current_user_assumptions[j]) {
        removed = previous_user_assumptions[i++];
      } else if (previous_user_assumptions[i] > current_user_assumptions[j]) {
        ++j;
      } else {
      ...
      if (removed) {
        current_diff_seed.push_back(removed);
        current_diff_seed.push_back(litNegation(removed));
      }
    }
  }

  for (int a : current_user_assumptions) {
    current_diff_seed.push_back(a);
    current_diff_seed.push_back(litNegation(a));
  }

  normalize_lit_vector(current_diff_seed);
  
  add_bound_hardening_to_diff_seed(current_diff_seed);
...
void CGSS2::undo_consequence(int consequence_id) {
  if (consequence_id < 0) return;
  if (consequence_id >= (int)consequences.size()) return;

  Consequence& c = consequences[consequence_id];

  if (!c.active) return;

  LB -= c.weight;

  for (auto it = c.wdelta.rbegin(); it != c.wdelta.rend(); ++it) {
    int lit = it->first;
    int64_t delta = it->second;

    add_weight(lit, -delta);
  }

  c.active = false;
}

std::vector<int> CGSS2::compute_consequence_closure(const std::vector<int>& seed_lits) {
  std::vector<int> closure;

  if (seed_lits.empty()) return closure;
  if (consequences.empty()) return closure;

  std::unordered_set<int> seen_consequence;
  std::unordered_set<int> seen_lits;
  std::vector<int> lit_queue;

  lit_queue.reserve(seed_lits.size());

  for (int l : seed_lits) {
    if (l < 0) continue;

    if (seen_lits.insert(l).second) {
      lit_queue.push_back(l);
    }
  }

  for (unsigned qi = 0; qi < lit_queue.size(); ++qi) {
    int lit = lit_queue[qi];

    if (lit < 0) continue;
    if (lit >= (int)lit_to_consequences.size()) continue;

    for (int cid : lit_to_consequences[lit]) {
      if (cid < 0) continue;
      if (cid >= (int)consequences.size()) continue;

      Consequence& c = consequences[cid];

      if (!c.active) continue;
      if (!seen_consequence.insert(cid).second) continue;

      closure.push_back(cid);

      for (int l2 : consequence_index_lits[cid]) {
        if (l2 < 0) continue;

        if (seen_lits.insert(l2).second) {
          lit_queue.push_back(l2);
        }
      }
    }
  }
...
void CGSS2::undo_consequence_closure(const std::vector<int>& closure) {
  uint64_t before_lb = LB;

  std::vector<int> ordered = closure;
  std::sort(ordered.begin(), ordered.end(), [](int a, int b) { return a > b; });
  ordered.erase(std::unique(ordered.begin(), ordered.end()), ordered.end());

  save_replay_epoch_from_closure(ordered);

  for (int cid : ordered) {
    if (cid < 0) continue;
    if (cid >= (int)consequences.size()) continue;
    undo_consequence(cid);
  }
  ...
  for (int cid : ordered) {
    ...
    if (c.core_id >= 0 &&
        c.core_id < (int)core_to_consequence.size() &&
        core_to_consequence[c.core_id] == cid) {
      core_to_consequence[c.core_id] = -1;
    }


    if (cid < (int)consequence_index_lits.size()) {
      for (int lit : consequence_index_lits[cid]) {
        if (lit < 0) continue;
        if (lit >= (int)lit_to_consequences.size()) continue;

        std::vector<int>& bucket = lit_to_consequences[lit];
      ...
      std::vector<int>().swap(consequence_index_lits[cid]);
    }

    std::vector<int>().swap(c.core_lits);
    std::vector<int>().swap(c.added_lits);
    std::vector<int>().swap(c.removed_lits);
    std::vector<std::pair<int, int64_t>>().swap(c.wdelta);
  ...
  cleanup_bound_hardening_consequences();
}

void CGSS2::save_replay_epoch_from_closure(const std::vector<int>& closure) {
  if (closure.empty()) return;

  std::vector<int> chronological = closure;
  std::sort(chronological.begin(), chronological.end());

  std::vector<std::vector<int>> epoch;
  epoch.reserve(chronological.size());

  for (int cid : chronological) {
    if (cid < 0 || cid >= (int)consequences.size()) continue;
    const Consequence& c = consequences[cid];
    if (c.core_lits.empty()) continue;
    std::vector<int> normalized_core = c.core_lits;
    normalize_lit_vector(normalized_core);
    if (normalized_core.empty()) continue;
    epoch.push_back(std::move(normalized_core));
  }

  if (epoch.empty()) return;

  replay_core_history.push_back(std::move(epoch));
  while ((int)replay_core_history.size() > replay_core_history_epochs) {
    replay_core_history.pop_front();
  }
}

bool CGSS2::core_subset_of_assumptions(
    const std::vector<int>& core_lits,
    const std::vector<int>& sorted_active_assumptions
) const {
  auto it = sorted_active_assumptions.begin();
  auto end = sorted_active_assumptions.end();

  for (int l : core_lits) {
    it = std::lower_bound(it, end, l);
    if (it == end || *it != l) return false;
  }
  return true;
}

bool CGSS2::replay_saved_cores_for_assumptions(const std::vector<int>& active_assumptions) {
  if (replay_core_history.empty()) return true;

  std::vector<int> sorted_active_assumptions = active_assumptions;
  normalize_lit_vector(sorted_active_assumptions);

  for (auto epoch_it = replay_core_history.rbegin();
       epoch_it != replay_core_history.rend();
       ++epoch_it) {
    auto& epoch = *epoch_it;

    for (std::vector<int>& saved_core : epoch) {
      if (saved_core.empty()) continue;

      if (!core_subset_of_assumptions(saved_core, sorted_active_assumptions)) {
        continue;
      }

      int non_base = 0;
      uint64_t minw = HARDWEIGHT;
      for (int l : saved_core) {
        if (l <= 0) continue;
        if (is_base_assumption(l)) continue;
        if (!ws[l]) continue;
        ++non_base;
        if (ws[l] < minw) minw = ws[l];
      }
      if (!non_base || minw == HARDWEIGHT) {
        saved_core.clear();
        continue;
      }

      std::vector<int> replay_core = saved_core;
      if (push_core(replay_core)) {
        return false;
      }

      saved_core.clear();

      std::vector<int> refreshed_assumptions;
      if (!get_next_assumptions(refreshed_assumptions)) return false;
      sorted_active_assumptions = refreshed_assumptions;
      normalize_lit_vector(sorted_active_assumptions);
    }
  }

  while (!replay_core_history.empty()) {
    bool empty_epoch = true;
    for (const std::vector<int>& core : replay_core_history.front()) {
      if (!core.empty()) {
        empty_epoch = false;
        break;
      }
    }
    if (!empty_epoch) break;
    replay_core_history.pop_front();
  }
...
int CGSS2::apply_harden_in_consequence(
    int lit,
    bool soft_harden,
    int core_id,
    int consequence_id
  ...
  if (incremental && consequence_id < 0) {
    log.l(0) << "c bug: apply_harden_in_consequence without consequence\n";
    exit(-1);
  }

  if (soft_harden && hardening_lit) {
    consequence_add_added_lit(consequence_id, hardening_lit);
  }

  uint64_t nlit_w_before = ws[nlit];
  uint64_t lit_w_before = ws[lit];

  LB += nlit_w_before;

  if (!soft_harden) {
    in_core(nlit, core_id, consequence_id);
  } else {
    in_core(nlit, -1, consequence_id);
  }

  if (nlit_w_before) {
    consequence_add_weight_delta(consequence_id, nlit, -(int64_t)nlit_w_before);
    consequence_add_removed_lit(consequence_id, nlit);
  }

  if (lit_w_before) {
    consequence_add_weight_delta(consequence_id, lit, -(int64_t)lit_w_before);
    consequence_add_removed_lit(consequence_id, lit);
  }

  add_weight(nlit, -(int64_t)nlit_w_before);
  add_weight(lit,  -(int64_t)lit_w_before);

  wsb[nlit] = 0;
  wsb[lit] = 0;

  if ((bit_data[lit] & RELAX_LIT) &&
      !(bit_data[lit_to_clause[lit]] & SOFT_CL_IN_SOLVER)) {
    if (soft_harden && hardening_lit) {
      soft_clauses[lit_to_clause[lit]].push_back(litNegation(hardening_lit));
    }
    ...
    if (soft_harden && hardening_lit) {
      soft_clauses[lit_to_clause[lit]].pop_back();
    }
  } else {
    if (soft_harden && hardening_lit) {
...
void CGSS2::apply_lit_weight_update_now(int lit, uint64_t weight) {
  uint64_t old_wi = wi[lit];
  ...
  if (log.level(5)) {
    log.l(5) << "c apply_lit_weight_update_now " << lit
             << " old_wi=" << old_wi
             << " new_wi=" << wi[lit]
             << " matched_unit_soft=" << (int)seen
             << "\n";
  }
}

void CGSS2::consequence_register_lit(int consequence_id, int lit) {
  if (consequence_id < 0) return;
  if (lit < 0) return;

  reserve_vars(litVariable(lit));

  if ((int)consequence_index_lits.size() <= consequence_id) {
    consequence_index_lits.resize(consequence_id + 1);
  }

  std::vector<int>& registered = consequence_index_lits[consequence_id];

  if (std::find(registered.begin(), registered.end(), lit) != registered.end()) {
    return;
  }

  registered.push_back(lit);
  lit_to_consequences[lit].push_back(consequence_id);
}

void CGSS2::register_consequence_dependencies(int consequence_id) {
  if (consequence_id < 0) return;
  ...
  for (int l : c.core_lits) {
    consequence_register_lit(consequence_id, l);
  }

  for (int l : c.added_lits) {
    consequence_register_lit(consequence_id, l);
  }

  for (int l : c.removed_lits) {
    consequence_register_lit(consequence_id, l);
  }

  for (const auto& p : c.wdelta) {
    consequence_register_lit(consequence_id, p.first);
  }

  c.indexed = true;
}

int CGSS2::begin_replay_consequence(int core_id, const std::vector<int>& core_lits, uint64_t weight) {
  return begin_consequence(core_id, core_lits, weight);
}

int CGSS2::begin_consequence(int core_id, const std::vector<int>& core_lits, uint64_t weight) {
  Consequence c;
  c.id = (int)consequences.size();
  c.core_id = core_id;
  c.weight = weight;
  c.core_lits = core_lits;
  c.active = true;

  consequences.push_back(std::move(c));

  if ((int)consequence_index_lits.size() <= consequences.back().id) {
    consequence_index_lits.resize(consequences.back().id + 1);
  }

  if (core_id >= 0) {
    if ((int)core_to_consequence.size() <= core_id) {
      core_to_consequence.resize(core_id + 1, -1);
    }

    core_to_consequence[core_id] = consequences.back().id;
  }

  return consequences.back().id;
}

int CGSS2::consequence_for_core(int core_id) const {
  if (core_id < 0) return -1;
  if (core_id >= (int)core_to_consequence.size()) return -1;
  return core_to_consequence[core_id];
}

void CGSS2::consequence_add_weight_delta(int consequence_id, int lit, int64_t delta) {
  if (consequence_id < 0) return;
  consequences[consequence_id].wdelta.push_back({lit, delta});
}

void CGSS2::consequence_add_added_lit(int consequence_id, int lit) {
  if (consequence_id < 0) return;
  consequences[consequence_id].added_lits.push_back(lit);
}

void CGSS2::consequence_add_removed_lit(int consequence_id, int lit) {
  if (consequence_id < 0) return;
  consequences[consequence_id].removed_lits.push_back(lit);
}

void CGSS2::finish_consequence(int consequence_id) {
  if (consequence_id < 0) return;

  Consequence& c = consequences[consequence_id];

  register_consequence_dependencies(consequence_id);

  if (log.level(5)) {
    log.l(5) << "c consequence " << c.id
             << " core=" << c.core_id
             << " weight=" << c.weight
             << " core_size=" << c.core_lits.size()
             << " added=" << c.added_lits.size()
             << " removed=" << c.removed_lits.size()
             << " deltas=" << c.wdelta.size()
             << "\n";
  }
}
...
void CGSS2::in_core(int l, int i, int consequence_id) {
  ...
  if (bit_data[l]&OUTPUT_LIT) {
    ...
    if (t.outputs.back() == litNegation(l) && (int)t.outputs.size() < t.nof_inputs) {
      int toplit = vars<<1;
      int raw_nxt = encoder->next_output(litNegation(l), toplit);
      toplit_from_encoder(toplit);
      add_encoder_clauses();

      t.outputs.push_back(raw_nxt);

      if (t.minw < climit) {
        sumw -= t.minw;
      }

      int nxt = litNegation(raw_nxt);
      ...
      if (consequence_id >= 0) {
        consequence_add_weight_delta(consequence_id, nxt, (int64_t)t.minw);
        consequence_add_added_lit(consequence_id, nxt);
      } else if (incremental) {
        log.l(0) << "c bug: unlogged in_core output activation"
                 << " input=" << l
                 << " output=" << nxt
                 << " weight=" << t.minw
                 << "\n";
...
void CGSS2::remove_contradictory_assumptions() {
  for (int l : alit_order) {
    ...
    if (ws[nl] && !is_base_assumption(nl)) {
      uint64_t minw = min(w, ws[nl]);

      std::vector<int> synthetic_core = {l, nl};
      int consequence_id = begin_consequence(-1, synthetic_core, minw);

      consequence_add_weight_delta(consequence_id, l, -(int64_t)minw);
      consequence_add_weight_delta(consequence_id, nl, -(int64_t)minw);

      if (ws[l] == minw) {
        consequence_add_removed_lit(consequence_id, l);
      }

      if (ws[nl] == minw) {
        consequence_add_removed_lit(consequence_id, nl);
      }

      LB += minw;

      add_weight(l, -(int64_t)minw);
      add_weight(nl, -(int64_t)minw);

      finish_consequence(consequence_id);
    }
  }
  ...
  for (int l : base_assumptions) {
    ...
    if (ws[nl] && !is_base_assumption(nl)) {
      ...
      std::vector<int> synthetic_core = {l, nl};
      int consequence_id = begin_consequence(-1, synthetic_core, w);

      consequence_add_weight_delta(consequence_id, nl, -(int64_t)w);
      consequence_add_removed_lit(consequence_id, nl);

      LB += w;

      add_weight(nl, -(int64_t)w);
  ...
  if (LB) {
    log.l(1) << "c LB after removing contradictory assumptions: " << LB << "\n";
  }
}
...
void CGSS2::exhaust_totalizer(Totalizer& t, int bgi) {
  for (int i = bgi; i < (int)t.outputs.size(); ++i) {
    ...
    bool rv;
    if (incremental) {
      std::vector<int> assumps = base_assumptions;
      assumps.push_back(l);
      rv = satsolver->solve(assumps);
    } else {
      rv = satsolver->solve(l);
    }
    ...
    if (!rv) {
      ...
      int core_id;
      int consequence_id = -1;

      if (incremental) {
        std::vector<int> core_lits = base_assumptions;
        core_lits.push_back(l);
        normalize_lit_vector(core_lits);

        std::vector<int> core_snapshot = core_lits;
        core_id = found_core(core_lits, w);
        consequence_id = begin_consequence(core_id, core_snapshot, w);
      } else {
        core_id = found_core(l, w);
      }

      in_core(l, core_id, consequence_id);

      LB += w;

      if (incremental) {
        consequence_add_weight_delta(consequence_id, l, -(int64_t)w);
      ...
      add_weight(l, -(int64_t)w);
      wsb[l] = ws[l];

      if (incremental && hardening_lit) {
        satsolver->add_clause({litNegation(hardening_lit), litNegation(l)});
        if (upsolver) {
          upsolver->add_clause({litNegation(hardening_lit), litNegation(l)});
        }
      } else {
        satsolver->add_clause(litNegation(l));
        if (upsolver) {
          upsolver->add_clause(litNegation(l));
        }
...
int CGSS2::init_totalizer(const vector<int>& lits, uint64_t minw) {
  ...
  inputs.reserve(lits.size());

  for (int l : lits) {
    if (!is_base_assumption(l)) {
      inputs.push_back(litNegation(l));
    }
  }
  ...
  totalizers.emplace_back(inputs, minw);
...
bool CGSS2::relax_ss(std::vector<int>& core_set) {
  ...
  for (unsigned ii=0; ii<core_set.size(); ++ii) {
    ...
    int consequence_id = consequence_for_core(core_set[ii]);
    if (incremental && consequence_id < 0) {
      log.l(0) << "c bug: relax_ss has no consequence for core "
               << core_set[ii]
               << "\n";
      exit(-1);
    }
    if (late_in_core) {
      for (int l : c.lits) {
        in_core(l, core_set[ii], consequence_id);
      }
    }
    ...
    for (j = 1; j < t.outputs.size(); ++j) {
      int out_lit = litNegation(t.outputs[j]);

      add_weight(out_lit, minw);
      wsb[out_lit] = ws[out_lit];

      consequence_add_weight_delta(consequence_id, out_lit, (int64_t)minw);
      consequence_add_added_lit(consequence_id, out_lit);
    }
...
bool CGSS2::push_core(std::vector<int>& core) {
  ...
  if (nof_lits == 0) {
    ...
    bool soft_harden = core.size() > 1;
    int opposite_lit = litNegation(core_lit);

    vector<int> core_snapshot = core;
    int core_id = found_core(core, minw);
    int consequence_id = begin_consequence(core_id, core_snapshot, minw);

    apply_harden_in_consequence(
        opposite_lit,
        soft_harden,
        core_id,
        consequence_id
    ...
    log.l(2) << "c unit core";
    log.l(4) << " {" << core_lit << "}";
    log.l(2) << ", weight=" << minw << ", LB = " << LB;
    if (use_ub) log.l(2) << ", UB = " << UB;
    log.l(2) << "\n";
  } else {
    ...
    for (int l : core) {
      ...
      if (incremental && !late_in_core) {
        log.l(0) << "c bug: incremental partial backtracking requires late_in_core\n";
        exit(-1);
      }
    }
    ...
    int consequence_id = -1;
    if (!fc_core) {
      consequence_id = begin_consequence(core_id, core_snapshot, minw);
    }
    LB += minw;

    for (int l : core_snapshot)  {
      if (is_base_assumption(l)) continue;
    ...
    if (!fc_core) {
      ...
      cores_to_relax.push_back(core_id);
    } else {
      cores_to_relax_fc.push_back(core_id);
    }
...
int CGSS2::harden(int lit, bool soft_harden) {
  int nlit = litNegation(lit);
  uint64_t w = ws[nlit];
  ...
  if (!soft_harden) {
    core_id = found_core(nlit, w);
    deps = cores[core_id].lits;
  } else {
    deps.push_back(nlit);
    deps.push_back(lit);

    if (hardening_lit) {
      deps.push_back(hardening_lit);
    }

    normalize_lit_vector(deps);
  }

  int cid = begin_consequence(core_id, deps, w);

  apply_harden_in_consequence(lit, soft_harden, core_id, cid);

  finish_consequence(cid);

  if (soft_harden) {
    mark_bound_hardening_consequence(cid);
  }
...
void CGSS2::calc_conns() {
  ...
  for (int l : alit_order) {
    ...
    for (int tl : up) {
      if (tl != l) {
        if (ws[tl]) {
          connsp[l].push_back(tl);

          if (ordered_objectives &&
              !is_base_assumption(l) &&
              !is_base_assumption(tl)) {
            ordered_graph.add_edge(l, tl);
          }
        }

        if (ws[litNegation(tl)]) {
          connsn[l].push_back(tl);
        }
      }
    }
    ...
    for (int tl : up) {
      if (tl != nl) {
        if (ws[tl]) {
          connsp[nl].push_back(tl);

          if (ordered_objectives &&
              !is_base_assumption(nl) &&
              !is_base_assumption(tl)) {
            ordered_graph.add_edge(nl, tl);
          }
        }

        if (ws[litNegation(tl)]) {
          connsn[nl].push_back(tl);
        }
      }
    }
...
void CGSS2::maybe_enable_ordered_objective_stratification() {
  if (!ordered_objectives) return;
  if (!ordered_objective_strat) return;
  if (ordered_strat.active()) return;

  // Keep v1 conservative around WCE. Ordered stratification is a frontier policy.
  // Pending relaxations already define another frontier.
  if (!cores_to_relax.empty()) return;
  if (!cores_to_relax_fc.empty()) return;

  ordered_graph.finalize();

  OrderedObjectiveAnalyzer::Result r =
      ordered_analyzer.analyze(
          ordered_graph,
          alit_order,
          ws,
          [this](int l) { return this->is_base_assumption(l); }
      );

  if (!r.full_order) {
    if (log.level(4)) {
      log.l(4) << "c ordered objective not detected"
               << ", active_lits=" << r.active_lits
               << ", components=" << r.components
               << ", too_large=" << (int)r.too_large
               << "\n";
    }
    return;
  }

  ordered_strat.configure_full_order(r);

  if (ordered_strat.active()) {
    log.l(1) << "c ordered objective detected"
             << ", active_lits=" << r.active_lits
             << ", components=" << r.components
             << ", ordered_levels=" << ordered_strat.levels()
             << "\n";
  }
}
  ...
  while (!done) {
    ...
    for (unsigned i=0; i<lits.size(); ++i) {
      ...
      if (am1.size()>1) {
        ...
        if (!greedy_costs) {
          std::vector<int> am1_snapshot = am1;
          uint64_t penalty = minw * (am1_snapshot.size() - 1);

          int consequence_id = -1;
          if (incremental) {
            consequence_id = begin_consequence(-1, am1_snapshot, penalty);
          }

          LB += penalty;

          for (int l2 : am1_snapshot) {
            if (is_base_assumption(l2)) continue;

            if (incremental) {
              consequence_add_weight_delta(consequence_id, l2, -(int64_t)minw);

              if (ws[l2] == minw) {
                consequence_add_removed_lit(consequence_id, l2);
              }
            }

            add_weight(l2, -(int64_t)minw);
            wsb[l2] = ws[l2];

            if (incremental) {
              in_core(l2, -1, consequence_id);
            } else {
              in_core(l2);
            }
          }
          ...
          int rl = new_var();
          int relax_lit = posLit(rl);

          if (log.level(5)) {
            log.l(5) << "c am1: { ";
            for (int l2 : am1_snapshot) log.l(5) << l2 << " ";
            log.l(5) << "} relax lit: " << negLit(rl) << "\n";
          }

          am1.push_back(negLit(rl));

          add_weight(relax_lit, minw);
          wsb[relax_lit] = ws[relax_lit];

          if (incremental) {
            consequence_add_weight_delta(consequence_id, relax_lit, (int64_t)minw);
            consequence_add_added_lit(consequence_id, relax_lit);
          }
...
bool CGSS2::next_strat_level() {
  ...
  if (ordered_objectives && ordered_objective_strat && ordered_strat.active()) {
    if (cores_to_relax.empty() && cores_to_relax_fc.empty()) {
      if (ordered_strat.next_level()) {
        log.l(1) << "c ordered objective level "
                << ordered_strat.level() + 1
                << " / " << ordered_strat.levels()
                << "\n";
        return 1;
      }
    }

    ordered_strat.disable();
    log.l(2) << "c ordered objective stratification exhausted or interrupted\n";
  }
...
void CGSS2::extract_model(bool check_cost) {
  ...
  while ((int)model.size()<=vars) model.push_back(false);
...
void CGSS2::try_hardening() {
  ...
  bool use_ub2 = !ordered_objective_frontier_active();

  uint64_t UB2 = sumw;

  uint64_t pending_ss = 0;
  for (int ci : cores_to_relax) {
    Core& c = cores[ci];
    pending_ss += c.weight * (c.lits.size() - 1);
  }

  uint64_t pending_fc = 0;
  for (int ci : cores_to_relax_fc) {
    Core& c = cores[ci];
    uint64_t add = c.weight * (c.lits.size() - 1);
    for (int l : c.lits) add += cws[l];
    pending_fc += add;
  }
  ...
  // log.l(0) << "c UB2_BREAKDOWN"
  //         << " sumw=" << sumw
  //         << " pending_ss=" << pending_ss
  ...
  for (int l : alit_order) {
    ...
    bool by_gap = ws[l] > UB - LB;
    bool by_ub2 = use_ub2 && ws[l] > UB2;
  ...
  if (hardening == 2) {
    for (int l1 : alit_order) {
      ...
      for (int l2 : alit_order) {
        ...
        uint64_t pairw = ws[l1] + ws[l2];

        bool by_gap = pairw > UB - LB;
        bool by_ub2 = use_ub2 && pairw > UB2;

        if (by_gap || by_ub2) {
          bool query_local = incremental && hardening_lit;
          ...
          if (query_local) {
            ...
            if (upsolver) upsolver->add_clause({l1, l2, litNegation(hardening_lit)});
          } else {
            satsolver->add_clause({l1, l2});
            if (upsolver) upsolver->add_clause({l1, l2});
          }
        }
      }
    }
...
bool CGSS2::assumption_contains_lit(
    const std::vector<int>& assumptions,
    int lit
) const {
  return std::find(assumptions.begin(), assumptions.end(), lit) != assumptions.end();
}
...
bool CGSS2::get_next_assumptions(vector<int>& assumptions) {
  ...
  if (ordered_objectives && ordered_objective_strat && ordered_strat.active()) {
    for (int l : alit_order) {
      if (is_base_assumption(l)) continue;
      if (!ws[l]) continue;
      if (!ordered_strat.is_unlocked(l)) continue;
    ...
    std::stable_sort(
        assumptions.begin() + base_assumptions.size(),
        assumptions.end(),
        [this](int a, int b) {
          int la = ordered_strat.unlock_level(a);
          int lb = ordered_strat.unlock_level(b);

          if (la != lb) return la > lb;  // newest frontier first
          return ws[a] > ws[b];
        }
    );
  ...
  auto cmp_cost_decreasing = [this](int& a, int& b) {
    return ws[a] > ws[b];
  };

  if (order_assumptions) {
    sort(
        assumptions.begin() + base_assumptions.size(),
        assumptions.end(),
        cmp_cost_decreasing
...
bool CGSS2::flush_pending_relaxations() {
  if (cores_to_relax.empty() && cores_to_relax_fc.empty()) {
    return true;
  }

  unsigned saved_threshold = core_relax_threshold;
  core_relax_threshold = 0;

  while (!cores_to_relax.empty() || !cores_to_relax_fc.empty()) {
    bool relaxed = false;

    if (!cores_to_relax.empty()) {
      relaxed = relax(cores_to_relax, false) || relaxed;
    }

    if (!cores_to_relax_fc.empty()) {
      relaxed = relax(cores_to_relax_fc, true) || relaxed;
    }

    if (!relaxed) {
      core_relax_threshold = saved_threshold;
      return false;
    }
  }
...
int CGSS2::_solve() {
  ...
  if (weights.size()) {
    ...
    med_weight = (weights.size()&1) ? weights[weights.size()/2] :((long double)weights[weights.size()/2-1] + weights[weights.size()/2])/2;
  }
  if (AbstCG==1) {
  ...
  while (next_strat_level()) {
    ...
    std::vector<int> replay_assumptions;
    if (get_next_assumptions(replay_assumptions)) {
      if (!replay_saved_cores_for_assumptions(replay_assumptions)) {
        pop_clock();
        return 0;
      }
    }

    if (am1s || calc_conns_all || ordered_objectives) {
      calc_conns();
    }

    if (ordered_objectives) {
      maybe_enable_ordered_objective_stratification();
    }
    ...
    while (1) {
      ...
      if (rv) {
        ...
        if (use_ub) {
          extract_model(1);
          if (LB == UB) {
            log.l(1) << "c search ended after model extraction since LB==UB\n";
            pop_clock();
            return 1;
          }
        }
        ...
        //     << " size=" << core.size()
        //     << "\n";
        if (ordered_objectives && ordered_objective_core_min) {
          std::size_t old_size = core.size();

          ordered_graph.finalize();
          ordered_graph.minimize_core(core);

          if (log.level(3) && core.size() != old_size) {
            log.l(3) << "c ordered graph reduced core "
                    << old_size << " -> " << core.size()
                    << "\n";
          }
        }
...
void CGSS2::set_lit_weight(int lit, uint64_t weight, bool dimacs) {
  ...
  if (incremental) {
    queue_weight_update(lit, weight);
  } else {
    init_weight(lit, weight);
  }
}
...
void CGSS2::init(Options& op) {
  Log logi = log.l(1);
  // log.l(0) << "c initialize CGSS2 MaxSAT solver\n";
  // log.l(0) << "c " << version() << "\n";
  ...
  if (op.bools["ordered-objectives"]) {
    ordered_objectives = 1;
    logi << "c enable ordered objective heuristics\n";
  }

  if (op.bools["ordered-obj-no-strat"]) {
    ordered_objective_strat = 0;
    logi << "c disable ordered objective stratification\n";
  }

  if (op.bools["ordered-obj-no-core-min"]) {
    ordered_objective_core_min = 0;
    logi << "c disable ordered objective core minimization\n";
  }
...
void CGSS2::mark_bound_hardening_consequence(int consequence_id) {
  if (consequence_id < 0) return;
  if (consequence_id >= (int)consequences.size()) return;

  bound_hardening_consequences.push_back(consequence_id);

}

void CGSS2::add_bound_hardening_to_diff_seed(std::vector<int>& seed_lits) {
  if (bound_hardening_consequences.empty()) return;

  int active = 0;
  int added_lits = 0;

  for (int cid : bound_hardening_consequences) {
    if (cid < 0) continue;
    if (cid >= (int)consequences.size()) continue;
    ...
    if (cid < (int)consequence_index_lits.size() &&
        !consequence_index_lits[cid].empty()) {
      for (int l : consequence_index_lits[cid]) {
        if (l < 0) continue;
        seed_lits.push_back(l);
        ++added_lits;
      }
      continue;
    }
  ...
  if (active) {
    normalize_lit_vector(seed_lits);
  }
}

void CGSS2::cleanup_bound_hardening_consequences() {
  if (bound_hardening_consequences.empty()) return;
  ...
  for (int cid : bound_hardening_consequences) {
    if (cid < 0) continue;
    if (cid >= (int)consequences.size()) continue;
    if (!consequences[cid].active) continue;

    bound_hardening_consequences[j++] = cid;
  }

  bound_hardening_consequences.resize(j);

}

bool CGSS2::ordered_objective_frontier_active() const {
  return ordered_objectives &&
         ordered_objective_strat &&
         ordered_strat.active();
}
...
void CGSS2::add_options(Options& op) {
  ...
  op.add(Options::Option::BOOL, "ordered-objectives", "ordered-objectives", 0,
       "enable ordered objective implication graph heuristics", "0", 2);

  op.add(Options::Option::BOOL, "ordered-obj-no-strat", "ordered-obj-no-strat", 0,
        "disable ordered objective stratification", "0", 2);

  op.add(Options::Option::BOOL, "ordered-obj-no-core-min", "ordered-obj-no-core-min", 0,
        "disable ordered objective graph core minimization", "0", 2);
...
CGSS2::CGSS2() :
  ...
  incremental(1), optimizer_time(0), map_all_vars(0), keep_polarities(1), late_soft_cl_to_lit(0), lazy_add_softs(1), late_in_core(1), order_assumptions(1),
  multisolve_solves(1), strat(1), blo(1), distance_based_strat(0), exp_weight_strat(0), trim(1), minimize(2),
  ordered_objectives(1),
  ordered_objective_strat(1),
  ordered_objective_core_min(1),
  minimize_budget_absolute(1000), minimize_budget_relative(0.01), core_relax_threshold0(0), core_relax_threshold(0),
  core_relax_delay_strategy(0), exhaust(1), calc_conns_all(0), hardening(2), greedy_costs(0), am1s(1), am1s_reiter(1),
  ...
  climit(HARDWEIGHT), sumw(0), LB(0), UB(numeric_limits<uint64_t>::max()),
  replay_core_history_epochs(10),
  ...
  last_hardening_lit(0),
  stats_cores_relaxed_fc(0), stats_greedy_extra_cost(0),
  time_preprocess(0), time_relax(0), time_ss_relax_calls(0), time_trim_core(0),
...
const char* CGSS2::ipamir_signature() {
  static std::string ipamir_signature_string = "share-trail";
...
void CGSS2::ipamir_add_soft_lit(int32_t l, uint64_t w) {
  ...
  // CGSS2 stores the falsified-cost literal, hence -l.
  set_lit_weight(-l, w, true);
}
...
int32_t CGSS2::ipamir_solve() {
  ...
  coalesce_pending_weight_updates();

  std::vector<int> current_user_assumptions = base_assumptions;
  normalize_lit_vector(current_user_assumptions);

  compute_incremental_diff_seed(current_user_assumptions);

  if (last_hardening_lit) {
    current_diff_seed.push_back(last_hardening_lit);
    normalize_lit_vector(current_diff_seed);
  }

  std::vector<int> current_consequence_closure = compute_consequence_closure(current_diff_seed);

  undo_consequence_closure(current_consequence_closure);

  restore_incremental_assumption_weights(
      last_user_assumptions,
      current_user_assumptions
  );

  apply_pending_weight_updates();

  prepare_incremental_query_assumptions(current_user_assumptions);

  if (!replay_saved_cores_for_assumptions(base_assumptions)) {
    pop_clock();
    base_assumptions.clear();
    return 20;
  }
  ...
  if (incremental && (!cores_to_relax.empty() || !cores_to_relax_fc.empty())) {
    if (!flush_pending_relaxations()) {
      log.l(0) << "c bug: could not flush pending WCE relaxations at ipamir boundary\n";
      exit(-1);
    }
  }

  last_user_assumptions = current_user_assumptions;
