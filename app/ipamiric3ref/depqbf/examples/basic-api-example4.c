...
int main (int argc, char** argv)
{
  ...
  /* Add a fresh variable with 'id == 1' to the existential block. */
  qdpll_add_var_to_scope(depqbf, 1, 2);

  /* Add a fresh variable with 'id == 2' to the univeral block. */
  qdpll_add_var_to_scope(depqbf, 2, 1);
  ...
  printf ("added clause '2 1 0' to a new stack frame.\n");
  ...
  QDPLLAssignment a = qdpll_get_value (depqbf, 2);
  printf ("partial countermodel - value of 2: %s\n", a == QDPLL_ASSIGNMENT_UNDEF ? "undef" :
          (a == QDPLL_ASSIGNMENT_FALSE ? "false" : "true"));

  qdpll_reset (depqbf);

  /* Discard the clause '2 1 0' by popping off the topmost frame. */
  qdpll_pop (depqbf);

  printf ("discarding clause '2 1 0' by a 'pop'.\n");
  ...
  res = qdpll_sat (depqbf);
  /* The formula is satisfiable again because we discarded the clause '2 1 0'
     by a 'pop'. */
  assert (res == QDPLL_RESULT_SAT);
